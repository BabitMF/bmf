...
VideoSync::VideoSync(AVRational input_stream_time_base,
                     AVRational encode_time_base,
...
int VideoSync::process_video_frame(AVFrame *frame,
    ...
    if (!frame) {
        nb0_frames = nb_frames = mid_pred(
            last_nb0_frames_[0], last_nb0_frames_[1], last_nb0_frames_[2]);
        ...
        float_pts = av_rescale_q(frame->pts, input_stream_time_base_,
                                 temp_encode_time_base);
        //- (ost_start_time_ != AV_NOPTS_VALUE ?
        //   av_rescale_q(ost_start_time_, AV_TIME_BASE_Q,
        ...
        if (stream_start_time_ != AV_NOPTS_VALUE &&
            stream_first_dts_ != AV_NOPTS_VALUE && video_frame_rate_.num)
        ...
        case VSYNC_VSCFR:
            if (frame_number == 0 && delta0 >= 0.5) {
                BMFLOG(BMF_DEBUG) << "Not duplicating " << (int)lrintf(delta0)
                                  << "initial frames";
                delta = duration;
                delta0 = 0;
    ...
    memmove(last_nb0_frames_ + 1, last_nb0_frames_,
            sizeof(last_nb0_frames_[0]) *
                (FF_ARRAY_ELEMS(last_nb0_frames_) - 1));
