...
int dump_module(std::string module_name) {
  ...
  try {
    ...
    get_module_register_info =
        factory->module_info(info); // XXX: will throw an exception when dump a
  ...
  if (get_module_register_info) {
    std::cout << std::setw(w) << std::setfill(' ')
              << "module_description: " << info.module_description << std::endl;
    std::cout << std::setw(w) << std::setfill(' ')
...
int install_module(const bmf_sdk::ModuleInfo &info, bool force) {
  ...
  if (!fs::exists(module_root_path) and !fs::create_directories(module_root_path)) {
    BMFLOG(BMF_ERROR) << "create module root directory failed." << std::endl;
    return -1;
  }

  auto installed_path =
      module_root_path / ("Module_" + info.module_name);
...
std::map<std::string, std::function<void()>> help_infos{
    ...
    {"list",
     []() {
       std::cout << "\tlist\t\tlist all locally installed modules" << std::endl;
     }},
    {"dump",
     []() {
       std::cout << "\tdump\t\t<module_name>" << std::endl;
       std::cout << "\t\t\tdump installed module infos specified by module_name"
                 << std::endl;
     }},
    {"install",
     []() {
