...
int main() {
    ...
    while (!terminated) {
        ...
        if (nav_mode == NAV_NORMAL) {
            ...
            case ' ':
                if (menu_stack.top()->menu_type == MENU_MULTI &&
                    menu_stack.top()->select_option())
                    break;
            case 10:
            case KEY_ENTER:
                if (menu_stack.top()->get_selected_menu().menu_type ==
                    MENU_OPTION) {
                    selected_options = menu_stack.top()->selections;
                    menu_stack.top()->callback();
                    menu_stack.pop();
                } else if (menu_stack.top()->get_selected_menu().menu_type ==
                           MENU_BACK) {
                    menu_stack.pop();
                } else {
