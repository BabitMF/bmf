...
# # 1. Build BMF
# # Assuming we are in the workspace root
# if [ -f "bmf/build.sh" ]; then
#     echo "Building BMF..."
#     (cd bmf && ./build.sh)
...
#     (cd ../.. && ./build.sh)
# else
#     echo "BMF build.sh not found, assuming it's already built."
# fi

# 2. Ensure Python version is compatible with ComfyUI (> 3.11) and set up environment
...
if [ "$CURRENT_MAJOR" -lt 3 ] || { [ "$CURRENT_MAJOR" -eq 3 ] && [ "$CURRENT_MINOR" -lt 11 ]; }; then
