...
class Graph {
  public:
    ...
    int get_hungry_check_func(std::shared_ptr<Node> &root_node, int output_idx,
    ...
    int
    add_all_mirrors_for_output_stream(std::shared_ptr<OutputStream> &stream);
    ...
    int add_input_stream_packet(std::string const &stream_name, Packet &packet,
                                bool block = false);

    Packet poll_output_stream_packet(std::string const &stream_name,
                                     bool block = true);
