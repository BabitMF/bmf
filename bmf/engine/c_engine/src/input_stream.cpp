...
InputStream::InputStream(int stream_id, std::string const &identifier,
    ...
    : stream_id_(stream_id), identifier_(identifier), alias_(alias),
      notify_(notify), node_id_(node_id), throttled_cb_(throttled_cb),
      max_queue_size_(max_queue_size),
      queue_(std::make_shared<SafeQueue<Packet>>()) {
    queue_->set_identifier(identifier_);
}

InputStream::InputStream(int stream_id, StreamConfig &stream_config,
                         int node_id,
                         std::function<void(int, bool)> &throttled_cb,
                         size_t max_queue_size)
    : stream_id_(stream_id), identifier_(stream_config.get_identifier()),
      alias_(stream_config.get_alias()), notify_(stream_config.get_notify()),
      node_id_(node_id), throttled_cb_(throttled_cb),
      max_queue_size_(max_queue_size),
      queue_(std::make_shared<SafeQueue<Packet>>()) {
    queue_->set_identifier(identifier_);
}
...
Packet InputStream::pop_packet_at_timestamp(int64_t timestamp) {
    ...
    if (pkt.timestamp() == EOS or pkt.timestamp() == BMF_EOF) {
        ...
        BMFLOG_NODE(BMF_INFO, node_id_)
            << "eof processed, remove node from scheduler";
        // if (node_id_ >= 0)
...
bool InputStream::is_empty() { return queue_->empty(); }
...
int64_t InputStream::get_time_bounding() { return next_time_bounding_; }

int InputStream::get_id() { return stream_id_; }
...
bool InputStream::is_full() { return queue_->size() >= max_queue_size_; }

void InputStream::set_connected(bool connected) { connected_ = connected; }

bool InputStream::is_connected() { return connected_; }
...
bool InputStream::get_block() { return block_; }

void InputStream::set_block(bool block) { block_ = block; }

void InputStream::probe_eof(bool probed) { probed_ = true; }
END_BMF_ENGINE_NS
