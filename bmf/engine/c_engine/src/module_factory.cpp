...
ModuleInfo ModuleFactory::create_module(std::string module_name, int node_id,
                                        JsonParam &option,
    ...
    if (factory == nullptr) {
        throw std::runtime_error(
            fmt::format("create module {} failed", module_name));
    }
...
JsonParam ModuleFactory::create_and_get_subgraph_config(std::string module_info,
                                                        int node_id,
                                                        std::string option) {
    ...
    std::shared_ptr<Module> module;
    create_module(info.module_name, node_id, opt, info.module_type,
                  info.module_path, info.module_entry, module);
...
ModuleFactory::create_and_test_subgraph(std::string module_info, int node_id,
                                        std::string option) {
    auto tmp = nlohmann::json::parse(module_info);
    auto info = ModuleConfig(tmp);
    auto opt = JsonParam(nlohmann::json::parse(option));
    ...
    create_module(info.module_name, node_id, opt, info.module_type,
                  info.module_path, info.module_entry, module_instance);
