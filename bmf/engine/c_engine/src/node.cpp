...
Node::Node(int node_id, NodeConfig &node_config, NodeCallBack &node_callback,
...
#ifndef NO_TRACE
    TraceProcessEmitter trace_emitter =
        TraceProcessEmitter(PROCESSING, node_name_);
#endif
    ...
    callback.notify_cb = [this]() -> bool { return this->schedule_node(); };
    callback.node_is_closed_cb = [this]() -> bool { return this->is_closed(); };
...
bool Node::is_closed() { return state_ == NodeState::CLOSED; }
...
void Node::get_hungry_check_func(int input_idx,
...
bool Node::any_of_input_queue_full() {
    for (auto input_stream = input_stream_manager_->input_streams_.begin();
         input_stream != input_stream_manager_->input_streams_.end();
         input_stream++) {
        if (input_stream->second->is_full())
            return true;
    }
...
int Node::need_force_close() { return force_close_; }
...
int Node::process_node(Task &task) {
    ...
    // before close we should propagate eos pkt to the downstream
        ...
        dec_pending_task();
        BMFLOG_NODE(BMF_INFO, id_) << "meet eos, close the node and propagate "
                                      "eos pkt instead of node process";
    ...
    //    PyErr_Print();
    //    cleanup();
    //    throw std::runtime_error("[" + node_name_ + "] " + "Python error
    //    already set.");
    //}
...
1. while an input packet is added to input_stream, this is called by
input_stream_manager
2. while node is in PENDING or all input streams done, it's scheduled by
scheduler
*/
bool Node::schedule_node() {
#ifndef NO_TRACE
    TraceProcessEmitter trace_emitter =
        TraceProcessEmitter(SCHEDULE, node_name_);
#endif
    mutex_.lock();
    ...
    // for node that need force_close, check if all downstream nodes are closed
    // already
    ...
    if (need_force_close()) {
        if (all_downstream_nodes_closed()) {
            ...
            BMFLOG_NODE(BMF_INFO, id_)
                << "scheduling failed, all downstream node closed: " << type_;
            return false;
        }
    }
    ...
    bool result = false;
    if (is_source()) {
        Task task = Task(id_, input_stream_manager_->stream_id_list_,
                         output_stream_manager_->get_stream_id_list());
        if (infinity_node_) {
        ...
        if (node_output_updated_) {
            input_stream_manager_->output_stream_id_list_ =
                output_stream_manager_->get_stream_id_list();
...
int Node::get_scheduler_queue_id() { return scheduler_queue_id_; }

int Node::get_input_stream_manager(
    std::shared_ptr<InputStreamManager> &input_stream_manager) {
    input_stream_manager = input_stream_manager_;
    return 0;
}

int Node::get_input_streams(
...
int Node::get_output_stream_manager(
...
std::string Node::get_type() { return type_; }
...
int64_t Node::get_last_timestamp() { return last_timestamp_; }
END_BMF_ENGINE_NS
