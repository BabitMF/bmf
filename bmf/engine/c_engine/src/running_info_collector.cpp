...
bmf::SchedulerInfo
RunningInfoCollector::collect_scheduler_info(Scheduler *scheduler) {
    bmf::SchedulerInfo scheduler_info;
    scheduler_info.last_schedule_success_time =
        scheduler->last_schedule_success_time_;
    ...
    for (auto &q : scheduler->scheduler_queues_)
        scheduler_info.scheduler_queues.push_back(
            collect_scheduler_queue_info(q.get()));

    return scheduler_info;
}

bmf::SchedulerQueueInfo RunningInfoCollector::collect_scheduler_queue_info(
    SchedulerQueue *scheduler_q) {
    ...
    default:
        sq_info.state =
            std::to_string(int64_t(scheduler_q->paused_state_)) + " -> UNKNOWN";
    }
...
bmf::NodeRunningInfo RunningInfoCollector::collect_node_info(Node *node,
                                                             Graph *graph) {
    ...
    node_info.schedule_success_count =
        uint64_t(node->schedule_node_success_cnt_);
    ...
    auto f = [](Graph *graph, uint64_t node_id,
                uint32_t stream_id) -> uint64_t {
        if (graph == nullptr)
            return INT_MAX;
        for (auto &nd : graph->nodes_)
            for (auto &os : nd.second->output_stream_manager_->output_streams_)
                for (auto &is : os.second->mirror_streams_)
                    if (is.input_stream_manager_->node_id_ == node_id &&
                        is.stream_id_ == stream_id)
...
bmf::InputStreamInfo
RunningInfoCollector::collect_input_stream_info(InputStream *stream) {
...
bmf::OutputStreamInfo
RunningInfoCollector::collect_output_stream_info(OutputStream *stream) {
    bmf::OutputStreamInfo s_info;
    s_info.id = uint64_t(stream->stream_id_);
    s_info.name = stream->identifier_;

    for (auto &s : stream->mirror_streams_)
        s_info.down_streams.push_back(collect_input_stream_info(
            s.input_stream_manager_->input_streams_[s.stream_id_].get()));
...
bmf::TaskInfo RunningInfoCollector::collect_task_info(Task *task) {
    ...
    for (auto &s : task->inputs_queue_) {
        bmf::TaskStreamInfo ts_info = {.id = uint64_t(s.first)};
