...
namespace bmf::builder {
    ...
    namespace internal {
        ...
        public:
            RealStream(const std::shared_ptr<RealNode>& node, std::string name, std::string notify,
                       std::string alias);
        ...
        public:
            RealNode(const std::shared_ptr<RealGraph>& graph, int id, std::string alias, const bmf_sdk::JsonParam& option,
                     std::vector<std::shared_ptr<RealStream> > inputStreams, std::string const &moduleName,
                     ModuleType mduleType, std::string const &modulePath, std::string const &moduleEntry,
        ...
        private:
            ...
            public:
                ...
                std::map<long long, std::shared_ptr<bmf::BMFCallback> > callbackInstances_;
            };
        ...
        public:
            ...
            void GiveStreamAlias(std::shared_ptr<RealStream> stream, std::string const &alias);
    ...
    bmf::BMFModule
    GetModuleInstance(std::string const &moduleName, std::string const &option, ModuleType moduleType = Python,
    ...
    public:
        ...
        BMF_FUNC_VIS Node Module(const std::vector<Stream>& inStreams, std::string const &moduleName, ModuleType moduleType,
                                 ...
                                 std::string const &moduleEntry = "", InputManagerType inputStreamManager = Immediate,
                                 int scheduler = 0);

        BMF_FUNC_VIS Node
        CppModule(const std::vector<Stream>& inStreams, std::string const &moduleName, const bmf_sdk::JsonParam& option,
                  std::string const &alias = "", std::string const &modulePath = "",
                  std::string const &moduleEntry = "", InputManagerType inputStreamManager = Immediate,
                  int scheduler = 0);

        BMF_FUNC_VIS Node
        PythonModule(const std::vector<Stream>& inStreams, std::string const &moduleName, const bmf_sdk::JsonParam& option,
                     std::string const &alias = "", std::string const &modulePath = "",
                     std::string const &moduleEntry = "", InputManagerType inputStreamManager = Immediate,
                     int scheduler = 0);

        BMF_FUNC_VIS Node
        GoModule(const std::vector<Stream>& inStreams, std::string const &moduleName, const bmf_sdk::JsonParam& option,
                 std::string const &alias = "", std::string const &modulePath = "",
                 std::string const &moduleEntry = "", InputManagerType inputStreamManager = Immediate,
                 int scheduler = 0);

        BMF_FUNC_VIS Node Decode(const bmf_sdk::JsonParam& decodePara, std::string const &alias = "");

        BMF_FUNC_VIS Node
        EncodeAsVideo(const bmf_sdk::JsonParam& encodePara, std::string const &alias = "");

        BMF_FUNC_VIS Node
        EncodeAsVideo(Stream audioStream, const bmf_sdk::JsonParam& encodePara, std::string const &alias = "");

        BMF_FUNC_VIS Node
        FFMpegFilter(const std::vector<Stream>& inStreams, std::string const &filterName, bmf_sdk::JsonParam filterPara,
        ...
        template<typename T>
        BMF_FUNC_VIS Node Amix(std::vector<Stream> inStreams, T para, std::string const &alias = "");

        template<typename T>
        BMF_FUNC_VIS Node Overlay(std::vector<Stream> inStreams, T para, std::string const &alias = "");

        template<typename T>
        BMF_FUNC_VIS Node Concat(std::vector<Stream> inStreams, T para, std::string const &alias = "");

        BMF_FUNC_VIS Node Fps(int fps, std::string const &alias = "");

    private:
        BMF_FUNC_VIS Node
        ConnectNewModule(std::string const &alias, const bmf_sdk::JsonParam& option, const std::vector<Stream>& inputStreams,
                         std::string const &moduleName, ModuleType moduleType, std::string const &modulePath,
                         std::string const &moduleEntry, InputManagerType inputStreamManager, int scheduler);
    ...
    public:
        ...
        BMF_FUNC_VIS void SetInputStreamManager(InputManagerType inputStreamManager);
        ...
        BMF_FUNC_VIS Node
        CppModule(const std::vector<class Stream>& inStreams, std::string const &moduleName, const bmf_sdk::JsonParam& option,
                  std::string const &alias = "", std::string const &modulePath = "",
                  std::string const &moduleEntry = "", InputManagerType inputStreamManager = Immediate,
                  int scheduler = 0);

        BMF_FUNC_VIS Node
        PythonModule(const std::vector<class Stream>& inStreams, std::string const &moduleName, const bmf_sdk::JsonParam& option,
                     std::string const &alias = "", std::string const &modulePath = "",
                     std::string const &moduleEntry = "", InputManagerType inputStreamManager = Immediate,
                     int scheduler = 0);

        BMF_FUNC_VIS Node
        GoModule(const std::vector<class Stream>& inStreams, std::string const &moduleName, const bmf_sdk::JsonParam& option,
                 std::string const &alias = "", std::string const &modulePath = "",
                 std::string const &moduleEntry = "", InputManagerType inputStreamManager = Immediate,
                 int scheduler = 0);
        ...
        BMF_FUNC_VIS Node
        EncodeAsVideo(class Stream audioStream, const bmf_sdk::JsonParam& encodePara, std::string const &alias = "");

        BMF_FUNC_VIS Node
        FFMpegFilter(const std::vector<class Stream>& inStreams, std::string const &filterName, bmf_sdk::JsonParam filterPara,
                     std::string const &alias = "");
        ...
        template<typename T>
        BMF_FUNC_VIS Node Amix(std::vector<class Stream> inStreams, T para, std::string const &alias = "");

        template<typename T>
        BMF_FUNC_VIS Node Overlay(std::vector<class Stream> inStreams, T para, std::string const &alias = "");

        template<typename T>
        BMF_FUNC_VIS Node Concat(std::vector<class Stream> inStreams, T para, std::string const &alias = "");

        BMF_FUNC_VIS Node Fps(int fps, std::string const &alias = "");

    private:
        BMF_FUNC_VIS Node
        ConnectNewModule(std::string const &alias, const bmf_sdk::JsonParam& option, const std::vector<class Stream>& inputStreams,
                         std::string const &moduleName, ModuleType moduleType, std::string const &modulePath,
                         std::string const &moduleEntry, InputManagerType inputStreamManager, int scheduler);

        BMF_FUNC_VIS Node InternalFFMpegFilter(const std::vector<class Stream>& inStreams, std::string const &filterName,
                                               const bmf_sdk::JsonParam& filterPara, std::string const &alias = "");
    };
    ...
    public:
        ...
        BMF_FUNC_VIS std::map<int, std::vector<Packet> > ProcessPkts(std::map<int, std::vector<Packet> > inputPackets);
    ...
    public:
        ...
        BMF_FUNC_VIS bmf::BMFGraph Instantiate(bool dumpGraph = true, bool needMerge = true);
        ...
        BMF_FUNC_VIS Node Decode(const bmf_sdk::JsonParam& decodePara, Stream controlStream, std::string const &alias = "");

        BMF_FUNC_VIS Node
        Encode(Stream videoStream, Stream audioStream, const bmf_sdk::JsonParam& encodePara, std::string const &alias = "");

        BMF_FUNC_VIS Node
        Encode(Stream videoStream, const bmf_sdk::JsonParam& encodePara, std::string const &alias = "");

        BMF_FUNC_VIS Node
        Encode(const bmf_sdk::JsonParam& encodePara, std::string const &alias = "");

        BMF_FUNC_VIS Node
        FFMpegFilter(const std::vector<Stream>& inStreams, std::string const &filterName, const bmf_sdk::JsonParam& filterPara,
                     std::string const &alias = "");
        ...
        template<typename T>
        BMF_FUNC_VIS Node Vflip(Stream inStream, T para, std::string const &alias = "");

        template<typename T>
        BMF_FUNC_VIS Node Scale(Stream inStream, T para, std::string const &alias = "");

        template<typename T>
        BMF_FUNC_VIS Node Setsar(Stream inStream, T para, std::string const &alias = "");

        template<typename T>
        BMF_FUNC_VIS Node Pad(Stream inStream, T para, std::string const &alias = "");

        template<typename T>
        BMF_FUNC_VIS Node Trim(Stream inStream, T para, std::string const &alias = "");

        template<typename T>
        BMF_FUNC_VIS Node Setpts(Stream inStream, T para, std::string const &alias = "");

        template<typename T>
        BMF_FUNC_VIS Node Loop(Stream inStream, T para, std::string const &alias = "");

        template<typename T>
        BMF_FUNC_VIS Node Split(Stream inStream, T para, std::string const &alias = "");

        template<typename T>
        BMF_FUNC_VIS Node Adelay(Stream inStream, T para, std::string const &alias = "");

        template<typename T>
        BMF_FUNC_VIS Node Atrim(Stream inStream, T para, std::string const &alias = "");

        template<typename T>
        BMF_FUNC_VIS Node Afade(Stream inStream, T para, std::string const &alias = "");

        template<typename T>
        BMF_FUNC_VIS Node Asetpts(Stream inStream, T para, std::string const &alias = "");
        ...
        BMF_FUNC_VIS Node Fps(Stream inStream, int fps, std::string const &alias = "");
        ...
        BMF_FUNC_VIS std::map<int, std::vector<Packet> > Process(SyncModule module, 
                            std::map<int, std::vector<Packet> > inputPackets);

        BMF_FUNC_VIS SyncPackets Process(SyncModule module, SyncPackets pkts = SyncPackets());
