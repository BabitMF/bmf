...
namespace bmf {
BMFGraph::BMFGraph(const std::string &graph_config, bool is_path,
                   bool need_merge) {
    ...
    std::map<int, std::shared_ptr<bmf_engine::ModuleCallbackLayer>>
        callback_bindings;
    ...
    for (auto &nd : g_config.nodes) {
        auto meta = nd.meta;
        callback_bindings[nd.id] =
            std::make_shared<bmf_engine::ModuleCallbackLayer>();
        if (meta.get_premodule_id() > 0) {
            if (!internal::ConnectorMapping::ModuleInstanceMapping().exist(
                    meta.get_premodule_id()))
                throw std::logic_error("Trying to use an unexisted premodule.");
            created_modules[nd.id] =
                internal::ConnectorMapping::ModuleInstanceMapping().get(
                    meta.get_premodule_id());
        }

        for (auto cb : meta.get_callback_binding()) {
            if (!internal::ConnectorMapping::ModuleCallbackInstanceMapping()
                     .exist(cb.second))
    ...
    auto g_instance = std::make_shared<bmf_engine::Graph>(
        g_config, all_created_modules, callback_bindings);

    graph_uid_ =
        internal::ConnectorMapping::GraphInstanceMapping().insert(g_instance);
}
...
uint32_t BMFGraph::uid() { return graph_uid_; }
...
int BMFGraph::add_input_stream_packet(const std::string &stream_name,
                                      bmf_sdk::Packet &packet, bool block) {
    return internal::ConnectorMapping::GraphInstanceMapping()
        .get(graph_uid_)
        ->add_input_stream_packet(stream_name, packet, block);
}

bmf_sdk::Packet
BMFGraph::poll_output_stream_packet(const std::string &stream_name,
                                    bool block) {
    return internal::ConnectorMapping::GraphInstanceMapping()
        .get(graph_uid_)
        ->poll_output_stream_packet(stream_name, block);
}
...
BMFModule::BMFModule(const std::string &module_name, const std::string &option,
    ...
    auto m = bmf_engine::ModuleFactory::create_module(
        module_name, -1, opt, module_type, module_path, module_entry, mod);
    if (!mod) {
    ...
    module_uid_ =
        internal::ConnectorMapping::ModuleInstanceMapping().insert(mod);
}

BMFModule::BMFModule(std::shared_ptr<bmf_sdk::Module> module_p) {
    module_uid_ =
        internal::ConnectorMapping::ModuleInstanceMapping().insert(module_p);
}
...
uint32_t BMFModule::uid() { return module_uid_; }

int32_t BMFModule::process(bmf_sdk::Task &task) {
    BMF_TRACE(PROCESSING, module_name_.c_str(), START);
    int32_t status = internal::ConnectorMapping::ModuleInstanceMapping()
                         .get(module_uid_)
                         ->process(task);
...
BMFCallback::BMFCallback(
    ...
    callback_uid_ =
        internal::ConnectorMapping::ModuleCallbackInstanceMapping().insert(
            std::make_shared<std::function<bmf_sdk::CBytes(bmf_sdk::CBytes)>>(
                callback));
}

BMFCallback::BMFCallback(BMFCallback const &cb) {
    callback_uid_ = cb.callback_uid_;
    internal::ConnectorMapping::ModuleCallbackInstanceMapping().ref(
        callback_uid_);
}

BMFCallback::~BMFCallback() {
    internal::ConnectorMapping::ModuleCallbackInstanceMapping().remove(
        callback_uid_);
}
