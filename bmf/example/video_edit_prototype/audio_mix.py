...
class audio_mix(SubGraph):
    ...
    def create_graph(self, option=None):
        ...
        for (i, audio_stream) in enumerate(audio_streams):
            ...
            p_audio_stream = (audio_stream.atrim(end=info['duration']).adelay(
                '%d|%d' % (info['start'] * 1000, info['start'] * 1000)))
            p_audio_streams.append(p_audio_stream)

        output_stream = (bmf.amix(p_audio_streams,
                                  inputs=len(p_audio_streams),
                                  duration='longest'))
