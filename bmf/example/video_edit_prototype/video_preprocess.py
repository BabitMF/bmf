    ...
    def create_graph(self, option=None):
        ...
        if trims and len(trims) > 0:
            ...
            for i in range(len(trims)):
                ...
                v_video = source_v_list[i].ff_filter(
                    'trim', start=start, end=end).setpts('PTS-STARTPTS')
                trims_video_list.append(v_video)
            # concat all slice
            video_stream = bmf.concat(*trims_video_list,
                                      n=len(trims_video_list),
                                      v=1,
                                      a=0)
        ...
        if speed:
            video_stream = video_stream.setpts(
                str(1.0 / float(speed)) + '*PTS')

        # video duration according to the record
        video_stream = video_stream.ff_filter(
            'trim', start=0, end=option['initial_info']
            ['video_duration']).setpts('PTS-STARTPTS')

        # do scale
        position = option.get('Position')
        if position:
            video_stream = video_stream.scale(position['Width'],
                                              position['Height'])

        # do crop
        crop = option.get('Crop')
        if crop:
            video_stream = video_stream.ff_filter('crop', crop['Width'],
                                                  crop['Height'], crop['PosX'],
                                                  crop['PosY'])
