...
public:
    ...
    DataPtr(void *data,  Device device) 
        : DataPtr(data, dummyDeleter, device)
    {
    }
...
HMP_API void set_allocator(DeviceType device, Allocator *allocator, unsigned flags = 0);
...
#define HMP_DECLARE_ALLOCATOR(device, flags) HMP_DECLARE_TAG(__s##device##flags##Allocator)
#define HMP_IMPORT_ALLOCATOR(device, flags) HMP_IMPORT_TAG(__s##device##flags##Allocator)
