...
public:
    ...
    TensorOptions(ScalarType stype)
        : scalar_type_(stype)
    {
    }

    TensorOptions(const Device &device)
        : device_(device)
    {
    }

    TensorOptions(DeviceType device)
        : device_(device)
    {
    }
    ...
    void option(const ScalarType &dtype)
    {
        scalar_type(dtype);
    }

    void option(const Device &device_)
    {
        device(device_);
    }

    void option(const std::string &device_)
    {
        device(device_);
    }

    void option(const char* device_)
    {
        device(Device(device_));
    }

    void option(bool pinned)
    {
    ...
    static TensorOptions make(Args&&...args)
    {
...
public:
    ...
    TensorInfo(const Buffer &buffer, const SizeArray &shape, int64_t bufferOffset = 0);
    TensorInfo(const Buffer &buffer, const SizeArray &shape, const SizeArray &strides, int64_t bufferOffset = 0);

    void setSizesAndStrides(const SizeArray &shape, int64_t bufferOffset = 0);
    void setSizesAndStrides(const SizeArray &shape, const SizeArray &strides, int64_t bufferOffset = 0);
    ...
    {
        return TensorOptions(device()).scalar_type(scalar_type())
    ...
    { 
        HMP_REQUIRE(getScalarType<T>() == scalar_type(),
            "Invalid scalar type {}, expect {}", getScalarType<T>(), scalar_type());
        return static_cast<T*>(unsafe_data()); 
    }

    inline void* unsafe_data() const { return static_cast<char*>(buffer_.unsafe_data()) + bufferOffset_ * itemsize(); }
