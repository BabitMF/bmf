...
inline std::tuple<ScalarType, int64_t> from_cv_type(int type)
{
    ...
    switch(depth){
    #define MAP(C, S) case C: return std::make_tuple(S, cn);
...
{
    switch(dtype){
    #define MAP(C, S) case S: return CV_MAKETYPE(C, cn);
    HMP_FOR_ALL_CV_TYPES(MAP)
    #undef MAP
    default:
...
static Tensor from_cv_mat(const cv::Mat &mat)
{
    ...
    return from_buffer(
        DataPtr(static_cast<void*>(mat.data), [mat](void*){}, kCPU),
        dtype, sizes, strides);
}
...
{
    HMP_REQUIRE(tensor.device_type() == kCPU, "cv::Mat only support cpu tensor");
    HMP_REQUIRE(tensor.dim() - channel_last >= 2,
         "cv::Mat require dim >= 2, got {}", tensor.dim() - channel_last);
    if(channel_last){
        HMP_REQUIRE(tensor.stride(-2) == tensor.size(-1) && tensor.stride(-1) == 1,
           "cv::Mat require last two strides are contiguous, expect ({}, 1), got ({}, {})",
            tensor.size(-1), tensor.stride(-2), tensor.stride(-1));
    }
    else{
        HMP_REQUIRE(tensor.stride(-1) == 1, 
            "cv::Mat require last stride equal to 1, got {}", tensor.stride(-1));
    }

    int cn = channel_last ? tensor.size(-1) : 1;
    int type = to_cv_type(tensor.dtype(), cn);
    std::vector<int> sizes(tensor.shape().begin(), tensor.shape().end() - channel_last);
    std::vector<size_t> steps;
    for(int64_t i = 0; i < tensor.dim() - 1 - channel_last; ++i){
        steps.push_back(tensor.stride(i)*tensor.itemsize());
    }
    
    return cv::Mat(tensor.dim() - channel_last, sizes.data(),
         type, tensor.unsafe_data(), steps.data());
}
