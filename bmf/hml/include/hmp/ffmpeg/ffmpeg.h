...
{
    return PixelInfo((PixelFormat)codecpar.format,
...
template<typename T>
static PixelFormatDesc make_pixel_desc(const T &v)
{
...
static void assign_pixel_info(AVFrame &avf, const PixelInfo &pix_info)
{
    ...
    avf.color_trc = (AVColorTransferCharacteristic)pix_info.color_model().transfer_characteristic();
}
...
static void assign_pixel_info(AVCodecContext &avcc, const PixelInfo &pix_info)
{
    ...
    avcc.color_trc = (AVColorTransferCharacteristic)pix_info.color_model().transfer_characteristic();
}
...
{
public:
    VideoWriter(const std::string &fn, int width, int height, int fps, const PixelInfo &pix_info, int kbs=2000);
    ~VideoWriter();
...
static uint64_t av_hw_frames_ctx_to_stream(const AVBufferRef *hw_frames_ctx_buf, const char *func = "")
{
    if(hw_frames_ctx_buf){
        ...
        if(hw_device_ctx->type == AV_HWDEVICE_TYPE_CUDA){
            ...
            HMP_REQUIRE(cuda_device_ctx, "{}: invalid hwframe, no cuda device ctx found", func);
...
static Device av_hw_frames_ctx_to_device(const AVBufferRef *hw_frames_ctx_buf, const char *func = "")
{
    if(hw_frames_ctx_buf){
        auto hw_frames_ctx = (AVHWFramesContext*)hw_frames_ctx_buf->data;
        auto hw_device_ctx = hw_frames_ctx->device_ctx;
        ...
        if(hw_device_ctx->type == AV_HWDEVICE_TYPE_CUDA){
            ...
            HMP_REQUIRE(rc == CUDA_SUCCESS, 
                    "{}: get cuda device index failed with rc={}", func, rc);
...
static AVBufferRef* av_hw_frames_ctx_from_device(const Device &device,
     int width, int height, AVPixelFormat sw_format)
{
    if(device.type() == kCPU){
        return nullptr;
    }
    ...
    if(device.type() == kCUDA){
        ...
        auto rc = av_hwdevice_ctx_create(&hw_device_ctx, 
        ...
        AVHWFramesContext *av_hw_frames_ctx = (AVHWFramesContext *)hw_frame_ctx->data;
...
{
    switch(format){
        case AV_SAMPLE_FMT_U8: return std::make_tuple(false, kUInt8);
        case AV_SAMPLE_FMT_U8P: return std::make_tuple(true, kUInt8);
        case AV_SAMPLE_FMT_S16: return std::make_tuple(false, kInt16);
        case AV_SAMPLE_FMT_S16P: return std::make_tuple(true, kInt16);
        case AV_SAMPLE_FMT_S32: return std::make_tuple(false, kInt32);
        case AV_SAMPLE_FMT_S32P: return std::make_tuple(true, kInt32);
        case AV_SAMPLE_FMT_FLT: return std::make_tuple(false, kFloat32);
        case AV_SAMPLE_FMT_FLTP: return std::make_tuple(true, kFloat32);
        case AV_SAMPLE_FMT_DBL: return std::make_tuple(false, kFloat64);
        case AV_SAMPLE_FMT_DBLP: return std::make_tuple(true, kFloat64);
        default:
...
{
    switch(dtype){
        case kUInt8: return planer ? AV_SAMPLE_FMT_U8P : AV_SAMPLE_FMT_U8;
        case kInt16: return planer ? AV_SAMPLE_FMT_S16P : AV_SAMPLE_FMT_S16;
        case kInt32: return planer ? AV_SAMPLE_FMT_S32P : AV_SAMPLE_FMT_S32;
        case kFloat32: return planer ? AV_SAMPLE_FMT_FLTP : AV_SAMPLE_FMT_FLT;
        case kFloat64: return planer ? AV_SAMPLE_FMT_DBLP : AV_SAMPLE_FMT_DBL;
        default:
            HMP_REQUIRE(false, "unsupported AVSampleFormat from {}:{}", dtype, planer);
    }
}
...
static AVBufferRef* to_av_buffer(const Tensor &d)
{
    ...
    return av_buffer_create((uint8_t*)d.unsafe_data(),
                             d.nbytes(),
...
static TensorList from_audio_frame(const AVFrame *avf)
{
    ...
    HMP_REQUIRE(!is_video_frame(avf), "from_audio_frame: AVFrame contains no audio data");
    ...
    if(planer){
        ...
        for(int i = 0; i < nplane; ++i){
            ...
            DataPtr data(my_avf->extended_data[i], [=](void *p) {
                av_frame_free((AVFrame**)&my_avf); 
            }, kCPU);
            auto plane = from_buffer(std::move(data), dtype, {my_avf->nb_samples});
            HMP_REQUIRE(plane.nbytes() <= my_avf->linesize[0], "from_audio_frame: invalid AVFrame");
            planes.push_back(plane);
        }
    }
    else{
        AVFrame *my_avf = av_frame_clone(avf);  
        DataPtr data(my_avf->extended_data[0], [=](void *p) {
            av_frame_free((AVFrame**)&my_avf); 
        }, kCPU);
        auto plane = from_buffer(std::move(data), dtype, {my_avf->nb_samples, my_avf->channels});
        HMP_REQUIRE(plane.nbytes() <= my_avf->linesize[0], "from_audio_frame: invalid AVFrame");
        planes.push_back(plane);
    }
...
static AVFrame* to_audio_frame(const TensorList &planes, const AVFrame *avf_ref, bool use_clone = true)
{
    HMP_REQUIRE(avf_ref && !is_video_frame(avf_ref), "to_audio_frame: AVFrame contains no audio data");
    ...
    HMP_REQUIRE(dtype == planes[0].dtype(), 
        "to_audio_frame: dtype is not match with ref AVFrame, expect {}, got {}",
    ...
    if(planer){
        HMP_REQUIRE(planes[0].dim() == 1, 
            "to_audio_frame: expect plane data is 1D array data for planer audio frame");
        HMP_REQUIRE(planes.size() == channels,
            "to_audio_frame: nb_channels not matched, expect {}, got {}", channels, planes.size());
    }
    else{
        HMP_REQUIRE(planes[0].dim() == 2, 
            "to_audio_frame: expect plane data is 2D array data for interleave audio frame");
        HMP_REQUIRE(planes.size() == 1 && planes[0].size(1) == channels,
            "to_audio_frame: nb_channels not matched, expect {}, got {}", channels, planes[0].size(1));
    }

    if(planes.size() > FF_ARRAY_ELEMS(avf->buf)){
        auto nb_extended_buf = planes.size() - FF_ARRAY_ELEMS(avf->buf);
        avf->extended_buf = (AVBufferRef**)av_mallocz_array(nb_extended_buf, sizeof(*avf->extended_buf));
        avf->extended_data = (uint8_t**)av_mallocz_array(planes.size(), sizeof(*avf->extended_data));
    ...
    for (int i = 0; i < planes.size(); ++i){
        HMP_REQUIRE(planes[i].is_contiguous(), "to_audio_frame: only support contigous data");
...
static AVFrame* to_audio_frame(const TensorList &planes, AVSampleFormat format, uint64_t channel_layout)
{
    AVFrame avf_ref;
...
static Tensor get_video_plane(const AVFrame *avf, int plane, const PixelFormatDesc &pix_desc)
{
    HMP_REQUIRE(avf && plane < FF_ARRAY_ELEMS(avf->data) &&  avf->data[plane],
         "get_video_plane: Invalid AVFrame or plane index is out of range");
    HMP_REQUIRE(is_video_frame(avf), "get_video_plane: AVFrame contains no image data");
    ...
    if(pix_desc.defined()){
        ...
        shape = SizeArray{pix_desc.infer_height(avf->height, plane),
                          pix_desc.infer_width(avf->width, plane),
                          channels};
        strides = SizeArray{avf->linesize[plane] / itemsize, channels, 1};
    }
    else{ //unsupported pixel format
        dtype = kUInt8;
        shape = SizeArray{avf->height, //FIXME: invalid height, as we don't known exact height
                          abs(avf->linesize[plane]),    //fix negative stride
                          1};
        strides = SizeArray{avf->linesize[plane], 1, 1};
    }
    //Note: negative linesize(strides) is possible(ff_vflip), but its representation is different from tensor
    //need port above code if hmp decided to support negative strides

    //
    auto device = av_hw_frames_ctx_to_device(avf->hw_frames_ctx, "get_video_plane");

    //don't use av_buffer_ref, as avf->data[plane] != avf->buf[plane]
    AVFrame *myframe = av_frame_clone(avf);  
    DataPtr data(myframe->data[plane], [=](void *p) {
        av_frame_free((AVFrame**)&myframe); 
    }, device);
 ...
 * if frame.deivce() != kCPU, hw_frames_ctx info must be provided either by avf_ref->hw_frames_ctx or hw_frames_ctx 
 * if both avf_ref->hw_frames_ctx and hw_frames_ctx are provided, hw_frames_ctx will be used
 * 
 * @param frame 
 * @param avf_ref 
 * @param hw_frames_ctx 
...
static AVFrame* to_video_frame(const Frame &frame, 
    const AVFrame *avf_ref = nullptr, AVBufferRef *hw_frames_ctx = nullptr)
{
    ...
    if(hw_frames_ctx){
        ...
        HMP_REQUIRE(avf_device == frame.device(), 
            "to_video_frame: invalid frame on device {}, as hw_frame_ctx is specified as {}",
        ...
        if(frame.device().type() != kCPU){
            HMP_REQUIRE(avf_device == frame.device(), 
                "to_video_frame: invalid frame on device {}, as hw_frame_ctx from AVFrame is {}",
                 frame.device(), avf_device);
        }
    }
    ...
    if(!avf_ref){
        if(!frame.pix_desc().defined()){
            // Warning: to avoid magic result, we need avf_ref if pixel format is not supported
            // ref: sws_scale, usePal()
            HMP_WRN("Ref AVFrame is necessary if PixelFormat is not supported internally");
        }
        memset(avf, 0, sizeof(*avf));
    }
    ...
    for(int i = 0; i < frame.nplanes(); ++i){
        auto plane = frame.plane(i).view({frame.plane(i).size(0), -1}); //ensure shape is (height, width*channels)
        if(plane.stride(1) != 1){ 
            av_frame_free(&avf);
    ...
    if(!avf->extended_data){
        ...
        HMP_REQUIRE(avf->extended_data == avf->data, 
