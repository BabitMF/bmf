...
HMP_API TensorList frame_format(const TensorList &data, PixelFormat format, int width, int height, bool has_batch = false);
HMP_API TensorList frame_format(const TensorList &data, const PixelFormatDesc &pix_desc, int width, int height, bool has_batch = false);
HMP_API TensorList frame_format(const TensorList &data, PixelFormat format, bool has_batch = false);

HMP_API Tensor image_format(const Tensor &image, ChannelFormat cformat = kNHWC, bool batch_first = true);
HMP_API TensorList image_format(const TensorList &images, ChannelFormat cformat = kNHWC, bool batch_first = true);

HMP_API Tensor &yuv_to_rgb(Tensor &dst, const TensorList &src,
     const PixelInfo &pix_info, ChannelFormat cformat = kNCHW);
HMP_API Tensor yuv_to_rgb(const TensorList &src,
     const PixelInfo &pix_info, ChannelFormat cformat = kNCHW);
...
HMP_API Tensor &resize(Tensor &dst, const Tensor &src, 
    ImageFilterMode mode=ImageFilterMode::Bilinear, ChannelFormat cformat=kNCHW);
HMP_API Tensor resize(const Tensor &src, int width, int height, 
    ImageFilterMode mode=ImageFilterMode::Bilinear, ChannelFormat cformat=kNCHW);
HMP_API Tensor &rotate(Tensor &dst, const Tensor &src,
    ImageRotationMode mode=ImageRotationMode::Rotate90, ChannelFormat cformat=kNCHW);
HMP_API Tensor rotate(const Tensor &src,
    ImageRotationMode mode=ImageRotationMode::Rotate90, ChannelFormat cformat=kNCHW);
HMP_API Tensor &mirror(Tensor &dst, const Tensor &src,
    ImageAxis axis=ImageAxis::Vertical, ChannelFormat cformat=kNCHW);
HMP_API Tensor mirror(const Tensor &src,
    ImageAxis axis=ImageAxis::Vertical, ChannelFormat cformat=kNCHW);

HMP_API Tensor normalize(const Tensor &src, const Tensor &mean, const Tensor& std, ChannelFormat cformat=kNCHW);
HMP_API Tensor& normalize(Tensor &dst, const Tensor &src, const Tensor &mean, const Tensor &std, ChannelFormat cformat=kNCHW);

//
HMP_API Tensor &erode(Tensor &dst, const Tensor &src, const optional<Tensor> &kernel=nullopt,
    ChannelFormat cformat=kNCHW);
HMP_API Tensor erode(const Tensor &src, const optional<Tensor> &kernel=nullopt,
    ChannelFormat cformat=kNCHW);
HMP_API Tensor &dilate(Tensor &dst, const Tensor &src, const optional<Tensor> &kernel=nullopt,
    ChannelFormat cformat=kNCHW);
HMP_API Tensor dilate(const Tensor &src, const optional<Tensor> &kernel=nullopt,
    ChannelFormat cformat=kNCHW);
...
HMP_API Tensor &canny(Tensor &dst, const Tensor &src, 
    const Scalar &low_thresh, const Scalar &high_thresh,
    int64_t aperture_size = 3, bool l2_gradient = false,
    ChannelFormat cformat = kNCHW); 
HMP_API Tensor canny(const Tensor &src, 
    const Scalar &low_thresh, const Scalar &high_thresh,
    int64_t aperture_size = 3, bool l2_gradient = false,
    ChannelFormat cformat = kNCHW); 

// Filters
HMP_API Tensor &filter2d(Tensor &dst, const Tensor &src,
    const Tensor &kernel, const Scalar& delta=0, ChannelFormat cformat=kNCHW);
HMP_API Tensor filter2d(const Tensor &src,
    const Tensor &kernel, const Scalar& delta=0, ChannelFormat cformat=kNCHW);

HMP_API Tensor warp_perspective(const Tensor &src, int64_t width, int64_t height,
