...
public:
    ...
    Tensor as_strided(const SizeArray &shape, const SizeArray &strides, optional<int64_t> offset = nullopt) const;
    Tensor& as_strided_(const SizeArray &shape, const SizeArray &strides, optional<int64_t> offset = nullopt);
    ...
    Tensor slice(int64_t dim, int64_t start, optional<int64_t> end = nullopt, int64_t step = 1) const;
    ...
    inline int64_t size(int64_t dim) const 
    {
        ...
        HMP_REQUIRE(dim < this->dim(), "dim {} is out of range {}", dim, this->dim());
        return shape()[dim];
    }
    ...
    inline bool is_same(const Tensor &other) { return self_.get() == other.self_.get();}

    inline bool is_cpu() const { return self_->device_type() == kCPU; }
    inline bool is_cuda() const { return self_->device_type() == kCUDA; }

    inline Tensor cpu(bool non_blocking=false) const { return to(kCPU, non_blocking); }
...
HMP_API Tensor from_buffer(DataPtr &&data, ScalarType scalar_type, 
    const SizeArray &shape, const optional<SizeArray> &strides = nullopt);
HMP_API Tensor empty(const SizeArray &shape, const TensorOptions &options = {});
HMP_API Tensor empty_like(const Tensor &other, const optional<TensorOptions> &options = nullopt);
HMP_API Tensor zeros(const SizeArray &shape, const TensorOptions &options = {});
HMP_API Tensor zeros_like(const Tensor &other, const optional<TensorOptions> &options = nullopt);
HMP_API Tensor ones(const SizeArray &shape, const TensorOptions &options = {});
HMP_API Tensor ones_like(const Tensor &other, const optional<TensorOptions> &options = nullopt);
HMP_API Tensor arange(int64_t start, int64_t end, int64_t step=1, const TensorOptions &options = {});
...
HMP_API Tensor& concat(Tensor &out, const TensorList &tensors, int64_t axis = 0);
...
//
HMP_API Tensor fromfile(const std::string &fn,
                        ScalarType dtype = kFloat32, int64_t count = -1, int64_t offset = 0);
