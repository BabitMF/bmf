...
#define DEF_TYPE_ATTR(_, name) .value("k" #name, ::ScalarType::name)
...
#undef DEF_TYPE_ATTR
    ...
    py::class_<Device>(m, "Device", py::dynamic_attr())
        .def(py::init<>())
        .def(py::init<DeviceType, Device::Index>(), py::arg("device_type"),
             py::arg("index") = 0)
    ...
    py::class_<Stream>(m, "Stream", py::dynamic_attr())
        ...
        .def("__repr__", [](const Stream &stream) { return stringfy(stream); })
    ...
    m.def("create_stream", &create_stream, py::arg("device_type"),
          py::arg("flags") = 0);
