...
Event::Event() : Event(false, true, false) {}
...
Event::~Event() {
    if (is_created_) {
        HMP_CUDA_CHECK_WRN(
            cudaEventDestroy(reinterpret_cast<cudaEvent_t>(event_)));
    }
}
...
void Event::record(const optional<Stream> &stream_) {
    ...
    HMP_REQUIRE(
        ...
        device_index_, stream.device().index());
    HMP_CUDA_CHECK(
        cudaEventRecord(reinterpret_cast<cudaEvent_t>(event_),
...
void Event::block(const optional<Stream> &stream_) {
    ...
    if (is_created_) {
        auto err =
            cudaStreamWaitEvent(reinterpret_cast<cudaStream_t>(stream.handle()),
...
void Event::synchronize() const {
    if (is_created_) {
        HMP_CUDA_CHECK(
            cudaEventSynchronize(reinterpret_cast<cudaEvent_t>(event_)));
    }
}
...
float Event::elapsed(const Event &other) {
    ...
    float ms = 0;
    HMP_CUDA_CHECK(
        cudaEventElapsedTime(&ms, reinterpret_cast<cudaEvent_t>(event_),
