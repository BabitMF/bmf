...
const char *hmp_last_error() { return s_hmp_last_error.c_str(); }

/////// hmp_Scalar ///////////
hmp_Scalar hmp_scalar_float(double v) { return new Scalar(v); }

hmp_Scalar hmp_scalar_int(int64_t v) { return new Scalar(v); }

hmp_Scalar hmp_scalar_bool(bool v) { return new Scalar(v); }
...
int hmp_device_count(int device_type) {
    HMP_PROTECT(return device_count(DeviceType(device_type));)
    return 0;
}
...
bool hmp_stream_query(hmp_Stream stream) {
    HMP_PROTECT(return stream->query();)
    return false;
}

void hmp_stream_synchronize(hmp_Stream stream) {
    HMP_PROTECT(stream->synchronize();)
}
...
hmp_StreamGuard hmp_stream_guard_create(hmp_Stream stream) {
    HMP_PROTECT(return new StreamGuard(*stream);)
    return nullptr;
}
...
hmp_Tensor hmp_tensor_empty(const int64_t *shape, int ndim, int type,
                            const char *device, bool pinned_memory) {
    SizeArray vshape(shape, shape + ndim);
    auto options = TensorOptions((ScalarType)type)
                       .device(Device(device))
                       .pinned_memory(pinned_memory);
    HMP_PROTECT(return new Tensor(empty(vshape, options)););

    return nullptr;
}

hmp_Tensor hmp_tensor_arange(int64_t start, int64_t end, int64_t step, int type,
                             const char *device, bool pinned_memory) {
    auto options = TensorOptions((ScalarType)type)
                       .device(Device(device))
                       .pinned_memory(pinned_memory);
    HMP_PROTECT(return new Tensor(arange(start, end, step, options));)
    return nullptr;
}
...
const char *hmp_tensor_stringfy(hmp_Tensor tensor, int *size) {
    HMP_PROTECT(s_tensor_stringfy_str = stringfy(*tensor);
...
hmp_Tensor hmp_tensor_slice(const hmp_Tensor tensor, int64_t dim, int64_t start,
                            int64_t end, int64_t step) {
    HMP_PROTECT(return new Tensor(tensor->slice(dim, start, end, step));)
    return nullptr;
}

hmp_Tensor hmp_tensor_select(const hmp_Tensor tensor, int64_t dim,
                             int64_t index) {
    HMP_PROTECT(return new Tensor(tensor->select(dim, index));)
    return nullptr;
}

hmp_Tensor hmp_tensor_permute(const hmp_Tensor tensor, const int64_t *dims,
                              int ndim) {
    HMP_PROTECT(SizeArray vdims{dims, dims + ndim};
                return new Tensor(tensor->permute(vdims));)
    return nullptr;
}

hmp_Tensor hmp_tensor_squeeze(const hmp_Tensor tensor, int64_t dim) {
    HMP_PROTECT(return new Tensor(tensor->squeeze(dim));)
    return nullptr;
}

hmp_Tensor hmp_tensor_unsqueeze(const hmp_Tensor tensor, int64_t dim) {
    HMP_PROTECT(return new Tensor(tensor->unsqueeze(dim));)
    return nullptr;
}

hmp_Tensor hmp_tensor_to_device(const hmp_Tensor data, const char *device,
                                bool non_blocking) {
    HMP_PROTECT(return new Tensor(data->to(Device(device), non_blocking));)
    return nullptr;
}

hmp_Tensor hmp_tensor_to_dtype(const hmp_Tensor data, int dtype) {
    HMP_PROTECT(return new Tensor(data->to((ScalarType)dtype));)
    return nullptr;
}
...
hmp_ColorModel hmp_color_model(int cs, int cr, int cp, int ctc) {
    HMP_PROTECT(return new ColorModel((ColorSpace)cs, (ColorRange)cr,
                                      (ColorPrimaries)cp,
...
int hmp_color_model_space(const hmp_ColorModel cm) { return (int)cm->space(); }
...
hmp_PixelInfo hmp_pixel_info(int format, const hmp_ColorModel cm) {
    HMP_PROTECT(return new PixelInfo((PixelFormat)format, *cm);)
    return nullptr;
}

hmp_PixelInfo hmp_pixel_info_v1(int format, int cs, int cr) {
    HMP_PROTECT(return new PixelInfo((PixelFormat)format, (ColorSpace)cs,
                                     (ColorRange)cr);)
    return nullptr;
}

hmp_PixelInfo hmp_pixel_info_v2(int format, int cp, int ctc) {
    HMP_PROTECT(return new PixelInfo((PixelFormat)format, (ColorPrimaries)cp,
                                     (ColorTransferCharacteristic)ctc);)
    return nullptr;
}
...
/////////////////// hmp_Frame /////////////////
hmp_Frame hmp_frame_make(int width, int height, const hmp_PixelInfo pix_info,
                         const char *device) {
    HMP_PROTECT(return new Frame(width, height, *pix_info, Device(device));)
    return nullptr;
}

hmp_Frame hmp_frame_from_data(hmp_Tensor *data, int size,
                              const hmp_PixelInfo pix_info) {
    TensorList vdata;
    for (int i = 0; i < size; ++i) {
        vdata.push_back(*data[i]);
    }
    HMP_PROTECT(return new Frame(vdata, *pix_info);)
    return nullptr;
}

hmp_Frame hmp_frame_from_data_v1(hmp_Tensor *data, int size, int width,
                                 int height, const hmp_PixelInfo pix_info) {
    TensorList vdata;
    for (int i = 0; i < size; ++i) {
        vdata.push_back(*data[i]);
    }
    HMP_PROTECT(return new Frame(vdata, width, height, *pix_info);)
    return nullptr;
}
...
bool hmp_frame_defined(const hmp_Frame frame) { return *frame; }

const hmp_PixelInfo hmp_frame_pix_info(const hmp_Frame frame) {
    return (const hmp_PixelInfo)&frame->pix_info();
}

int hmp_frame_format(const hmp_Frame frame) { return (int)frame->format(); }

int hmp_frame_width(const hmp_Frame frame) { return frame->width(); }

int hmp_frame_height(const hmp_Frame frame) { return frame->height(); }

int hmp_frame_dtype(const hmp_Frame frame) { return (int)frame->dtype(); }
...
int64_t hmp_frame_nplanes(const hmp_Frame frame) { return frame->nplanes(); }

const hmp_Tensor hmp_frame_plane(const hmp_Frame frame, int64_t p) {
    return &frame->plane(p);
}

hmp_Frame hmp_frame_to_device(const hmp_Frame frame, const char *device,
                              bool non_blocking) {
    HMP_PROTECT(return new Frame(frame->to(Device(device), non_blocking));)
    return nullptr;
}

void hmp_frame_copy_from(hmp_Frame self, const hmp_Frame from) {
    HMP_PROTECT(self->copy_(*from);)
}

hmp_Frame hmp_frame_clone(const hmp_Frame frame) {
    HMP_PROTECT(return new Frame(frame->clone());)
    return nullptr;
}

hmp_Frame hmp_frame_crop(const hmp_Frame frame, int left, int top, int width,
                         int height) {
    HMP_PROTECT(return new Frame(frame->crop(left, top, width, height));)
    return nullptr;
}

hmp_Frame hmp_frame_reformat(const hmp_Frame frame,
                             const hmp_PixelInfo pix_info) {
    HMP_PROTECT(return new Frame(frame->reformat(*pix_info));)
    return nullptr;
}
