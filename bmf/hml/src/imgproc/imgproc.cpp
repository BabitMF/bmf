...
TensorList frame_format(const TensorList &data, const PixelFormatDesc &pix_desc,
                        int width, int height, bool has_batch) {
    ...
    for (int i = 0; i < data.size(); ++i) {
        SizeArray shape{pix_desc.infer_height(height, i),
                        pix_desc.infer_width(width, i), pix_desc.channels(i)};
        if (has_batch) {
            shape.insert(shape.begin(), data[i].size(0));
        }
        HMP_REQUIRE(data[i].dtype() == pix_desc.dtype(),
                    "Expect {} for pixel format {}, got {}", pix_desc.dtype(),
                    pix_desc.format(), data[i].dtype());
...
TensorList frame_format(const TensorList &data, PixelFormat format, int width,
                        int height, bool has_batch) {
    return frame_format(data, PixelFormatDesc(format), width, height,
                        has_batch);
}

TensorList frame_format(const TensorList &data, PixelFormat format,
                        bool has_batch) {
...
Tensor image_format(const Tensor &image, ChannelFormat cformat,
                    bool batch_first) {
    if (image.dim() == 4) {
        return image;
...
TensorList image_format(const TensorList &images, ChannelFormat cformat,
                        bool batch_first) {
    TensorList vimages;
    for (auto &img : images) {
...
TensorList &yuv_to_yuv(TensorList &dst, const TensorList &src,
    ...
    HMP_REQUIRE(src_format != dst_format,
                "yuv_to_yuv: src and dst format cannot be the same.");

    return kernel::yuv_to_yuv(dst, src, dst_format, src_format);
}

TensorList yuv_to_yuv(const TensorList &src, const PixelInfo &dpix_info,
                      const PixelInfo &spix_info) {
    TensorList dst;
    auto has_batch_dim = src[0].dim() == 4;
    ...
    int shift = 0;
    if ((dpix_info.format() == PF_NV12 || dpix_info.format() == PF_NV12 ||
         dpix_info.format() == PF_P010LE) &&
        (spix_info.format() == PF_YUV420P ||
         spix_info.format() == PF_YUV420P10LE)) {
        shift = 0;
    } else if ((dpix_info.format() == PF_YUV420P ||
                dpix_info.format() == PF_YUV420P10LE) &&
               (spix_info.format() == PF_NV12 ||
                spix_info.format() == PF_NV12 ||
                spix_info.format() == PF_P010LE)) {
        shift = 1;
    } else {
        HMP_REQUIRE(false, "Unsupport PixelInfo");
    }
    if (has_batch_dim) {
        dst.push_back(
            empty({src[0].size(0), height, width, 1}, src[0].options()));
        dst.push_back(empty(
            {src[0].size(0), (height + 1) >> shift, (width + 1) >> shift, 1},
            src[0].options()));
        dst.push_back(empty(
            {src[0].size(0), (height + 1) >> shift, (width + 1) >> shift, 1},
            src[0].options()));
        ...
        dst.push_back(empty({(height + 1) >> shift, (width + 1) >> shift, 1},
                            src[0].options()));
        dst.push_back(empty({(height + 1) >> shift, (width + 1) >> shift, 1},
                            src[0].options()));
    }
...
TensorList &yuv_mirror(TensorList &dst, const TensorList &src,
                       const PixelInfo &pix_info, ImageAxis axis) {
    auto format = infer_ppixel_format(pix_info);
    return kernel::yuv_mirror(dst, src, format, axis);
}

Tensor &resize(Tensor &dst, const Tensor &src, ImageFilterMode mode,
               ChannelFormat cformat) {
    return kernel::img_resize(dst, src, mode, cformat);
}

Tensor resize(const Tensor &src, int width, int height, ImageFilterMode mode,
              ChannelFormat cformat) {
    auto dshape = src.shape();
    auto wdim =
        (cformat == kNCHW || src.dim() == 2) ? src.dim() - 1 : src.dim() - 2;
    auto hdim = wdim - 1;
    dshape[wdim] = width;
    dshape[hdim] = height;
...
Tensor &rotate(Tensor &dst, const Tensor &src, ImageRotationMode mode,
               ChannelFormat cformat) {
...
Tensor rotate(const Tensor &src, ImageRotationMode mode,
    ...
    auto wdim =
        (cformat == kNCHW || src.dim() == 2) ? src.dim() - 1 : src.dim() - 2;
    auto hdim = wdim - 1;
    if (mode == ImageRotationMode::Rotate90 ||
...
Tensor &mirror(Tensor &dst, const Tensor &src, ImageAxis axis,
               ChannelFormat cformat) {
...
Tensor normalize(const Tensor &src, const Tensor &mean, const Tensor &std,
                 ChannelFormat cformat) {
    auto dst = empty_like(src, src.options().dtype(kFloat32));
    return normalize(dst, src, mean, std, cformat);
}

Tensor &normalize(Tensor &dst, const Tensor &src, const Tensor &mean,
                  const Tensor &std, ChannelFormat cformat) {
    return kernel::img_normalize(dst, src, mean, std, cformat);
}

Tensor &erode(Tensor &dst, const Tensor &src, const optional<Tensor> &kernel_,
              ChannelFormat cformat) {
    Tensor kernel;
    if (kernel_) {
...
Tensor erode(const Tensor &src, const optional<Tensor> &kernel_,
             ChannelFormat cformat) {
    auto dst = empty_like(src);
    return erode(dst, src, kernel_, cformat);
}

Tensor &dilate(Tensor &dst, const Tensor &src, const optional<Tensor> &kernel_,
               ChannelFormat cformat) {
    Tensor kernel;
    if (kernel_) {
        kernel = kernel_.value();
...
Tensor dilate(const Tensor &src, const optional<Tensor> &kernel_,
              ChannelFormat cformat) {
    auto dst = empty_like(src);
...
Tensor sobel(const Tensor &src, int64_t dx, int64_t dy, int64_t ksize,
             const Scalar &scale, const Scalar &delta, ChannelFormat cformat) {
    auto dst = empty_like(src);
    return sobel(dst, src, dx, dy, ksize, scale, delta, cformat);
}

Tensor &canny(Tensor &dst, const Tensor &src, const Scalar &low_thresh,
              const Scalar &high_thresh, int64_t aperture_size,
              bool l2_gradient, ChannelFormat cformat) {
    return kernel::img_canny(dst, src, low_thresh, high_thresh, aperture_size,
                             l2_gradient, cformat);
}

Tensor canny(const Tensor &src, const Scalar &low_thresh,
             const Scalar &high_thresh, int64_t aperture_size, bool l2_gradient,
             ChannelFormat cformat) {
    auto shape = remove_cdim(src, cformat);
    auto dst = empty(shape, src.options());
    canny(dst, src, low_thresh, high_thresh, aperture_size, l2_gradient,
          cformat);
    dst.squeeze_();
    return dst;
}

Tensor &filter2d(Tensor &dst, const Tensor &src, const Tensor &kernel,
                 const Scalar &delta, ChannelFormat cformat) {
    return kernel::img_filter2d(dst, src, kernel, delta, cformat);
}

Tensor filter2d(const Tensor &src, const Tensor &kernel, const Scalar &delta,
                ChannelFormat cformat) {
    auto dst = empty_like(src);
    return filter2d(dst, src, kernel, delta, cformat);
}

Tensor bilateral_filter(const Tensor &src, int d, const Scalar &sigma_color,
                        const Scalar &sigma_space, ChannelFormat cformat) {
    auto dst = empty_like(src);
...
Tensor &bilateral_filter(Tensor &dst, const Tensor &src, int d,
                         const Scalar &sigma_color, const Scalar &sigma_space,
                         ChannelFormat cformat) {
    return kernel::img_bilateral_filter(dst, src, d, sigma_color, sigma_space,
                                        cformat);
}

Tensor gaussian_blur(const Tensor &src, int kx, int ky, const Scalar &sigma_x,
                     const Scalar &sigma_y, ChannelFormat cformat) {
    auto dst = empty_like(src);
    return gaussian_blur(dst, src, kx, ky, sigma_x, sigma_y, cformat);
}

Tensor &gaussian_blur(Tensor &dst, const Tensor &src, int kx, int ky,
                      const Scalar &sigma_x, const Scalar &sigma_y,
                      ChannelFormat cformat) {
    return kernel::img_gaussian_blur(dst, src, kx, ky, sigma_x, sigma_y,
                                     cformat);
}

Tensor warp_perspective(const Tensor &src, int64_t width, int64_t height,
                        const Tensor &M, ImageFilterMode mode,
                        ChannelFormat cformat) {
    auto wdim = infer_wdim(src, cformat);
    auto dshape = src.shape();
    dshape[wdim] = width;
...
Tensor &warp_perspective(Tensor &dst, const Tensor &src, const Tensor &M,
                         ImageFilterMode mode, ChannelFormat cformat) {
    return kernel::img_warp_perspective(dst, src, M, mode, cformat);
}

// dst = src0 * (1 - alpha) + src1 * (alpha);
Tensor &overlay(Tensor &dst, const Tensor &src0, const Tensor &src1,
                const Tensor &alpha) {
...
Tensor transfer(const Tensor &src, const ChannelFormat &src_format,
                const ChannelFormat &dst_format) {
    HMP_REQUIRE(src.dim() == 3 || src.dim() == 4, "dims must be 3 or 4");

    Tensor dst;
    if (src.dim() == 3) {
        if (dst_format == ChannelFormat::NCHW &&
            src_format == ChannelFormat::NHWC) {
            dst = src.permute({2, 0, 1});
        } else if (dst_format == ChannelFormat::NHWC &&
                   src_format == ChannelFormat::NCHW) {
