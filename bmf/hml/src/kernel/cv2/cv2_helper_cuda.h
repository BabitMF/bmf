...
static cv::cuda::GpuMat to_cv_gpu_mat(const Tensor &tensor,
                                      bool channel_last = false) {
    HMP_REQUIRE(tensor.device_type() == kCUDA,
                "cv::GpuMat only support cuda tensor");
    HMP_REQUIRE(tensor.dim() - channel_last == 2,
                "cv::GpuMat require dim == 2, got {}",
                tensor.dim() - channel_last);
    if (channel_last) {
        HMP_REQUIRE(tensor.stride(-2) == tensor.size(-1) &&
                        tensor.stride(-1) == 1,
                    "cv::GpuMat require last two strides are contiguous, "
                    "expect ({}, 1), got ({}, {})",
                    tensor.size(-1), tensor.stride(-2), tensor.stride(-1));
    } else {
        HMP_REQUIRE(tensor.stride(-1) == 1,
                    "cv::GpuMat require last stride equal to 1, got {}",
                    tensor.stride(-1));
    }

    int cn = channel_last ? tensor.size(-1) : 1;
    int type = to_cv_type(tensor.dtype(), cn);
    auto rows = tensor.size(0), cols = tensor.size(1);
...
static Tensor pitch_align_2d(const Tensor &t, bool copy = true,
                             ChannelFormat cformat = ChannelFormat::NCHW) {
    HMP_REQUIRE(cformat == ChannelFormat::NCHW,
                "pitch_align_2d only support NCHW layout");
...
static void morph_cuda(cv::MorphTypes algo, const cv::cuda::GpuMat &src,
                       cv::cuda::GpuMat &dst, cv::Mat &kernel) {
