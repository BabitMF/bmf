...
template <typename T> struct ImageIter {
    using value_type = T;
    using index_type = int;

    ImageIter() = default;

    HMP_HOST_DEVICE ImageIter(value_type *ptr, int width, int height,
                              int row_stride)
        : ptr_(ptr), width_(width), height_(height), row_stride_(row_stride) {}
...
template <typename Pixel> struct DefaultPixelValue {
    HMP_HOST_DEVICE static Pixel value() { return Pixel(); }
};
...
template <typename T> struct ImageIndexer {
    using index_type = T;

    ImageIndexer() = default;

    ImageIndexer(int batch, int width, int height, int batch_stride,
                 int row_stride)
        : batch_(batch), width_(width), height_(height),
          batch_stride_(batch_stride), row_stride_(row_stride) {}

    HMP_HOST_DEVICE inline index_type index(int batch, int x, int y) const {
        return index_type(batch) * batch_stride_ + index_type(y) * row_stride_ +
               x;
    }

    HMP_HOST_DEVICE int batch() const { return batch_; }

    HMP_HOST_DEVICE int width() const { return width_; }

    HMP_HOST_DEVICE int height() const { return height_; }

    int batch_stride_ = 0, row_stride_ = 0;
    int batch_ = 0, width_ = 0, height_ = 0;
};

template <typename Pixel, ChannelFormat Format = kNHWC> struct ImageSeqIter;
...
struct ImageSeqIter<Pixel, kNHWC> : public ImageIndexer<int> {
    ...
    ImageSeqIter(const Tensor &t, ChannelFormat format = kNHWC,
                 ImageBorderType border = kBReplicate) {
        HMP_REQUIRE(t.stride(-1) == 1,
                    "ImageSeqIter require last dim stride is 1, got {}",
                    t.stride(-1));
        ...
        if (format == kNCHW) {
            ...
            HMP_REQUIRE(
                Pixel::size() == 1,
                "ImageSeqIter invalid Vector type, expect size=1, got {}",
                Pixel::size());
            ...
            HMP_REQUIRE(t.dim() == 4, "ImageSeqIter require 4 dims, got {}",
                        t.dim());
            HMP_REQUIRE(t.stride(2) == t.size(3),
                        "ImageSeqIter require Vec(-2) dim stride is "
                        "contiguous, expect {}, got {}",
                        t.size(3), t.stride(2));
            HMP_REQUIRE(
                Pixel::size() == t.size(3),
                "ImageSeqIter invalid Vector type, expect size={}, got {}",
                t.size(-1), Pixel::size());
    ...
    static ImageSeqIter from_tensor(const Tensor &t,
                                    ChannelFormat format = kNCHW,
                                    ImageBorderType border = kBReplicate) {
...
struct ImageSeqIter<Pixel, kNCHW> : public ImageIndexer<int> {
    ...
    ImageSeqIter(const Tensor &t, ChannelFormat format = kNCHW,
                 ImageBorderType border = kBReplicate) {
        ...
        if (t.dim() == 4) {
            HMP_REQUIRE(t.size(1) == Pixel::size(),
                        "ImageSeqIter internal error");
...
struct YUVIter<
    T, format,
    std::enable_if_t<
        format == PPixelFormat::H420 || format == PPixelFormat::H422 ||
        format == PPixelFormat::H444 || format == PPixelFormat::I420 ||
        format == PPixelFormat::I422 || format == PPixelFormat::I444 ||
        format == PPixelFormat::U420 || format == PPixelFormat::U422 ||
        format == PPixelFormat::U444>> {
    const static int hshift = format == PPixelFormat::H420 ||
                              format == PPixelFormat::I420 ||
                              format == PPixelFormat::U420;
    const static int wshift =
        format == PPixelFormat::H420 || format == PPixelFormat::H422 ||
        format == PPixelFormat::I420 || format == PPixelFormat::I422 ||
        format == PPixelFormat::U420 || format == PPixelFormat::U422;
    ...
    YUVIter(const TensorList &yuv)
        ...
        HMP_REQUIRE(uiter.width_ == uv_width && uiter.height_ == uv_height,
                    "YUVIter: U plane size is not matched PixelFormat {}, "
                    "expect {}, got {}",
                    format, SizeArray{uv_width, uv_height},
                    SizeArray{uiter.width_, uiter.height_});
        HMP_REQUIRE(viter.width_ == uv_width && viter.height_ == uv_height,
                    "YUVIter: V plane size is not matched PixelFormat {}, "
                    "expect {}, got {}",
                    format, SizeArray{uv_width, uv_height},
                    SizeArray{viter.width_, viter.height_});
    }
    ...
    HMP_HOST_DEVICE inline void set(int batch, int w, int h,
                                    const Vector<T, 3> &yuv) {
    ...
    HMP_HOST_DEVICE inline int width() const { return yiter.width(); }

    HMP_HOST_DEVICE inline int height() const { return yiter.height(); }
};
...
struct YUVIter<T, format, std::enable_if_t<format == PPixelFormat::NV21 ||
    ...
    YUVIter(const TensorList &yuv)
        ...
        HMP_REQUIRE(uviter.width_ == uv_width && uviter.height_ == uv_height,
                    "YUVIter: UV plane size is not matched PixelFormat {}, "
                    "expect {}, got {}",
                    format, SizeArray{uv_width, uv_height},
                    SizeArray{uviter.width_, uviter.height_});
    }
...
#define HMP_DISPATCH_IMAGE_CHANNEL(channel, name, ...)                         \
    [&]() {                                                                    \
        ...
        default:                                                               \
            HMP_REQUIRE(false,                                                 \
                        "Unsupported image channels {} in {}, expect 1, 3, 4", \
                        channel, #name);                                       \
        }                                                                      \
    }()
