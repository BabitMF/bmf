...
std::string stringfy(const PPixelFormat &format) {
    switch (format) {
#define STRINGFY_CASE(name)                                                    \
    case PPixelFormat::name:                                                   \
        return "k" #name;
...
static inline void yuv_common_check(const TensorList &tensors, size_t idx,
                                    const std::string &name) {
...
static inline void yuv_common_check(const TensorList &dst,
                                    const TensorList &src,
                                    const std::string &name) {
    HMP_REQUIRE(dst.size() == src.size(),
                "{}: expect src and dst have same planes, got src={}, dst={}",
...
Tensor &yuv_to_rgb(Tensor &dst, const TensorList &src, PPixelFormat pformat,
    ...
    HMP_REQUIRE(dtmp.size(cdim) == 3,
                "yuv_to_rgb: require 3 channels for dtmp, got {}",
                dtmp.size(cdim));
...
TensorList &yuv_rotate(TensorList &dst, const TensorList &src,
    ...
    if (rotate == ImageRotationMode::Rotate0 ||
        rotate == ImageRotationMode::Rotate180) {
        HMP_REQUIRE(
            dtmp[0].size(1) == stmp[0].size(1) &&
                dtmp[0].size(2) == stmp[0].size(2),
            "yuv_rotate: image size are not matched with rotatation mode");
    } else if (rotate == ImageRotationMode::Rotate90 ||
               rotate == ImageRotationMode::Rotate270) {
        HMP_REQUIRE(
            dtmp[0].size(1) == stmp[0].size(2) &&
                dtmp[0].size(2) == stmp[0].size(1),
            "yuv_rotate: image size are not matched with rotatation mode");
    } else {
...
Tensor &img_resize(Tensor &dst, const Tensor &src, ImageFilterMode mode,
                   ChannelFormat cformat) {
    auto stmp = img::image_format(src, cformat);
    auto dtmp = img::image_format(dst, cformat);
    ...
    HMP_REQUIRE(stmp.size(0) == dtmp.size(0),
                "image_resize: expect same size of batch dim, src={}, dst={}",
                stmp.shape(), dtmp.shape());
    auto cdim = cformat == kNCHW ? 1 : -1;
    HMP_REQUIRE(stmp.size(cdim) == dtmp.size(cdim),
                "image_resize: expect same size of channel dim, src={}, dst={}",
                stmp.shape(), dtmp.shape());
...
Tensor &img_rotate(Tensor &dst, const Tensor &src, ImageRotationMode mode,
                   ChannelFormat cformat) {
    auto stmp = img::image_format(src, cformat);
    auto dtmp = img::image_format(dst, cformat);
...
Tensor &img_mirror(Tensor &dst, const Tensor &src, ImageAxis axis,
                   ChannelFormat cformat) {
    auto stmp = img::image_format(src, cformat);
    auto dtmp = img::image_format(dst, cformat);
    ...
    HMP_REQUIRE(
        stmp.shape() == dtmp.shape(),
        "img_mirror: expect src and dst have same shape, got src={}, dst={}",
        stmp.shape(), dtmp.shape());
...
Tensor &img_normalize(Tensor &dst, const Tensor &src, const Tensor &mean,
    ...
    HMP_REQUIRE(
        stmp.shape() == dtmp.shape(),
        "img_normalize: expect src and dst have same shape, got src={}, dst={}",
        stmp.shape(), dtmp.shape());
    auto cdim = cformat == kNCHW ? 1 : -1;
    HMP_REQUIRE(mean.dim() == 1 && std.dim() == 1 &&
                    mean.size(0) == std.size(0) &&
                    mean.size(0) == stmp.size(cdim),
                "img_normalize: invalid mean or std shape, expect ({},)",
                stmp.size(cdim));
...
Tensor &img_erode(Tensor &dst, const Tensor &src, const Tensor &kernel,
    ...
    HMP_REQUIRE(
        stmp.shape() == dtmp.shape(),
        "img_erode: expect src and dst have same shape, got src={}, dst={}",
        stmp.shape(), dtmp.shape());
...
Tensor &img_dilate(Tensor &dst, const Tensor &src, const Tensor &kernel,
    ...
    HMP_REQUIRE(
        stmp.shape() == dtmp.shape(),
        "img_dilate: expect src and dst have same shape, got src={}, dst={}",
        stmp.shape(), dtmp.shape());
...
Tensor &img_sobel(Tensor &dst, const Tensor &src, int64_t dx, int64_t dy,
    ...
    HMP_REQUIRE(
        stmp.shape() == dtmp.shape(),
        "img_sobel: expect src and dst have same shape, got src={}, dst={}",
        stmp.shape(), dtmp.shape());

    img_sobel_stub(stmp.device_type(), dtmp, stmp, dx, dy, ksize, scale, delta,
                   cformat);

    return dst;
}

Tensor &img_canny(Tensor &dst, const Tensor &src, const Scalar &low_thresh,
    ...
    HMP_REQUIRE(dtmp.size(cdim) == 1,
                "img_canny: invalid dst shape, expect 1 channel, got {}",
                stmp.size(cdim));
...
Tensor &img_warp_perspective(Tensor &dst, const Tensor &src, const Tensor &M,
    ...
    img_warp_perspective_stub(stmp.device_type(), dtmp, stmp, M, mode, cformat);

    return dst;
}

Tensor &img_filter2d(Tensor &dst, const Tensor &src, const Tensor &kernel,
    ...
    HMP_REQUIRE(
        stmp.shape() == dtmp.shape(),
        "img_filter2d: expect src and dst have same shape, got src={}, dst={}",
        stmp.shape(), dtmp.shape());
...
Tensor &img_overlay(Tensor &dst, const Tensor &src0, const Tensor &src1,
    ...
    HMP_REQUIRE(dst.dim() == 4,
                "img_overlay: expect dst tensor have 4 dims, got {}",
                dst.dim());
    checkShape({src0, src1}, dst.shape(), "img_overlay");
    HMP_REQUIRE(alpha.dim() >= 2,
                "img_overlay: expect alpha tensor have at least 2 dims, got {}",
                alpha.dim());
