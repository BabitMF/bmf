...
template <size_t N, typename T, size_t... I> struct make_integer_sequence_impl {
    using type =
        typename make_integer_sequence_impl<N - 1, T, N - 1, I...>::type;
};
...
using make_index_sequence =
    typename make_integer_sequence_impl<N, size_t>::type;

template <typename T> static inline T divup(T n, T d) {
    static_assert(std::is_integral<T>::value, "need integer type");
    return (n + d - 1) / d;
}

template <typename U, typename T> HMP_HOST_DEVICE inline U cast(const T &v) {
...
struct SaturateCast<Vector<U, Size>, Vector<T, Size>> {
    HMP_HOST_DEVICE inline static Vector<U, Size>
    cast(const Vector<T, Size> &v) {
        Vector<U, Size> r;
#pragma unroll
...
template <typename T> struct IntDivider {
...
template <> struct IntDivider<uint32_t> {
    ...
    IntDivider(uint32_t d) : d_(d) {
        ...
        HMP_REQUIRE(m <= std::numeric_limits<uint32_t>::max(),
                    "Internal error");
        m_ = m;
    }
...
struct OffsetCalculator {
    ...
    OffsetCalculator(unsigned ndim, const int64_t *sizes,
                     const int64_t **strides)
        : ndim_(ndim) {
        HMP_REQUIRE(ndim <= MaxDims, "Tensor has to many dims(<{}), dim={}",
                    MaxDims, ndim);
...
inline void checkContiguous(const TensorList &tensors,
                            const std::string &func) {
    for (size_t i = 0; i < tensors.size(); ++i) {
        HMP_REQUIRE(tensors.at(i).is_contiguous(),
                    "{}: only support contiguous tensor, got non-contiguous "
                    "tensor at {}",
                    func, i);
    }
}

inline void checkDevice(const TensorList &tensors, const Device &device,
                        const std::string &func) {
    for (size_t i = 0; i < tensors.size(); ++i) {
...
inline void checkShape(const TensorList &tensors, const SizeArray &shape,
                       const std::string &func) {
    for (size_t i = 0; i < tensors.size(); ++i) {
