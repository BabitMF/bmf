...
template <typename T>
void nppiResize(Tensor &dst, const Tensor &src, int mode,
                NppStreamContext &ctx) {
    HMP_REQUIRE(false, "resize_cuda: unsupported scalar type {}",
                getScalarType<T>());
}

template <>
void nppiResize<uint8_t>(Tensor &dst, const Tensor &src, int mode,
                         NppStreamContext &ctx) {
...
template <>
void nppiResize<uint16_t>(Tensor &dst, const Tensor &src, int mode,
                          NppStreamContext &ctx) {
...
template <>
void nppiResize<float>(Tensor &dst, const Tensor &src, int mode,
                       NppStreamContext &ctx) {
...
template <typename T>
void nppiRotate(Tensor &dst, const Tensor &src, ImageRotationMode mode,
                NppStreamContext &ctx) {
    HMP_REQUIRE(false, "rotate_cuda: unsupported scalar type {}",
                getScalarType<T>());
}

template <>
void nppiRotate<uint8_t>(Tensor &dst, const Tensor &src, ImageRotationMode mode,
                         NppStreamContext &ctx) {
    using scalar_t = uint8_t;
    ...
    auto status = nppiRotate_8u_C1R_Ctx(
        src.data<scalar_t>(), sSize, src.stride(0) * sizeof(scalar_t), sRoi,
        dst.data<scalar_t>(), dst.stride(0) * sizeof(scalar_t), dRoi, angle,
        xshift, yshift, NPPI_INTER_NN, ctx);
    HMP_REQUIRE(status >= NPP_SUCCESS,
                "nppiRotate_8u_C1R_Ctx failed with status={}", status);
}

template <>
void nppiRotate<uint16_t>(Tensor &dst, const Tensor &src,
                          ImageRotationMode mode, NppStreamContext &ctx) {
    using scalar_t = uint16_t;
    ...
    auto status = nppiRotate_16u_C1R_Ctx(
        src.data<scalar_t>(), sSize, src.stride(0) * sizeof(scalar_t), sRoi,
        dst.data<scalar_t>(), dst.stride(0) * sizeof(scalar_t), dRoi, angle,
        xshift, yshift, NPPI_INTER_NN, ctx);
    HMP_REQUIRE(status >= NPP_SUCCESS,
...
template <>
void nppiRotate<float>(Tensor &dst, const Tensor &src, ImageRotationMode mode,
                       NppStreamContext &ctx) {
    using scalar_t = float;
    ...
    auto status = nppiRotate_32f_C1R_Ctx(
        src.data<scalar_t>(), sSize, src.stride(0) * sizeof(scalar_t), sRoi,
        dst.data<scalar_t>(), dst.stride(0) * sizeof(scalar_t), dRoi, angle,
        xshift, yshift, NPPI_INTER_NN, ctx);
    HMP_REQUIRE(status >= NPP_SUCCESS,
...
template <typename T>
void nppiMirror(Tensor &dst, const Tensor &src, ImageAxis,
                NppStreamContext &ctx) {
    HMP_REQUIRE(false, "mirror_cuda: unsupported scalar type {}",
                getScalarType<T>());
}

template <>
void nppiMirror<uint8_t>(Tensor &dst, const Tensor &src, ImageAxis axis,
                         NppStreamContext &ctx) {
    using scalar_t = uint8_t;

    auto flip = getNPPAxis(axis);
    NppiSize oROI{.width = (int)dst.size(1), .height = (int)dst.size(0)};
    auto status = nppiMirror_8u_C1R_Ctx(
        src.data<scalar_t>(), src.stride(0) * sizeof(scalar_t),
        dst.data<scalar_t>(), dst.stride(0) * sizeof(scalar_t), oROI, flip,
        ctx);
    HMP_REQUIRE(status >= NPP_SUCCESS,
...
template <>
void nppiMirror<uint16_t>(Tensor &dst, const Tensor &src, ImageAxis axis,
                          NppStreamContext &ctx) {
    using scalar_t = uint16_t;

    auto flip = getNPPAxis(axis);
    NppiSize oROI{.width = (int)dst.size(1), .height = (int)dst.size(0)};
    auto status = nppiMirror_16u_C1R_Ctx(
        src.data<scalar_t>(), src.stride(0) * sizeof(scalar_t),
        dst.data<scalar_t>(), dst.stride(0) * sizeof(scalar_t), oROI, flip,
        ctx);
    HMP_REQUIRE(status >= NPP_SUCCESS,
...
template <>
void nppiMirror<float>(Tensor &dst, const Tensor &src, ImageAxis axis,
                       NppStreamContext &ctx) {
    using scalar_t = float;

    auto flip = getNPPAxis(axis);
    NppiSize oROI{.width = (int)dst.size(1), .height = (int)dst.size(0)};
    auto status = nppiMirror_32f_C1R_Ctx(
        src.data<scalar_t>(), src.stride(0) * sizeof(scalar_t),
        dst.data<scalar_t>(), dst.stride(0) * sizeof(scalar_t), oROI, flip,
        ctx);
    HMP_REQUIRE(status >= NPP_SUCCESS,
