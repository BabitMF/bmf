...
template <typename T, int Size> struct alignas(T) Vector {
    using value_type = T;
...
#pragma unroll
    ...
    HMP_HOST_DEVICE static constexpr int size() { return Size; }
    ...
    HMP_HOST_DEVICE inline value_type &operator[](int i) { return data[i]; }
...
#define DEFINE_INPLACE_BOP(op)                                                 \
    template <typename U>                                                      \
    HMP_HOST_DEVICE inline Vector &operator op##=(const U &v) {                \
        _Pragma("unroll") for (int i = 0; i < Size; ++i) { data[i] op## = v; } \
        return *this;                                                          \
    }                                                                          \
    template <typename U>                                                      \
    HMP_HOST_DEVICE inline Vector &operator op##=(const Vector<U, Size> &v) {  \
...
#define DEFINE_BOP(op)                                                         \
    ...
    HMP_HOST_DEVICE inline auto operator op(const Vector<T, Size> &v,          \
                                            const Vector<U, Size> &s) {        \
    ...
    template <typename T, typename U, int Size>                                \
    HMP_HOST_DEVICE inline auto operator op(const U &s,                        \
