...
std::string Tensor::repr() const {
    if (defined()) {
        return fmt::format("Tensor({}, {}, {})", device(), scalar_type(),
                           shape());
    } else {
...
Tensor &Tensor::fill_(const Scalar &value) { return hmp::fill(*this, value); }
...
Tensor Tensor::to(ScalarType dtype) const {
    if (this->dtype() == dtype) {
        ...
        auto tmp = empty_like(*this, this->options().dtype(dtype));
        copy(tmp, *this);
        return tmp;
    }
}
...
Tensor Tensor::as_strided(const SizeArray &shape, const SizeArray &strides,
...
Tensor &Tensor::as_strided_(const SizeArray &shape, const SizeArray &strides,
                            optional<int64_t> offset_) {
...
Tensor &Tensor::squeeze_(optional<int64_t> dim) {
    if (dim) {
        auto result =
            inferSqueezeGeometry(*this, wrap_size(dim.value(), this->dim()));
...
Tensor Tensor::alias() const {
    auto out = makeRefPtr<TensorInfo>(tensorInfo()->buffer(), shape(),
                                      strides(), tensorInfo()->bufferOffset());
...
Tensor Tensor::view(const SizeArray &shape_) const {
    ...
    HMP_REQUIRE(strides_, "can not view tensor as {} from {}", shape_,
                this->shape());
...
Tensor Tensor::transpose(int64_t dim0, int64_t dim1) const {
    ...
    HMP_REQUIRE(dim0 < dim(), "transpose: dim0({}) is out of range {}", dim0,
                dim());
    HMP_REQUIRE(dim1 < dim(), "transpose: dim1({}) is out of range {}", dim1,
                dim());
...
Tensor Tensor::permute(const SizeArray &dims) const {
    ...
    for (size_t i = 0; i < this->dim(); ++i) {
        ...
        HMP_REQUIRE(dim < this->dim(),
                    "permute: invalid dim={} at {}, expect less than {}", dim,
                    i, this->dim());
        HMP_REQUIRE(!flags[dim], "permute: duplicate dim={} at {} found", dim,
                    i);
...
Tensor Tensor::slice(int64_t dim, int64_t start, optional<int64_t> end_,
                     int64_t step) const {
    dim = wrap_size(dim, this->dim());
    HMP_REQUIRE(dim < this->dim(), "slice: dim({}) is out of range {}", dim,
                this->dim());
    ...
    HMP_REQUIRE(start < size(dim),
                "slice: start {} is out of range, need less than {}", start,
                size(dim));
    HMP_REQUIRE(end <= size(dim),
                "slice: start {} is out of range, need less or equal to {}",
                end, size(dim));
    auto size = (end + step - 1 - start) / step; // divup
    HMP_REQUIRE(
        size > 0,
        "slice: expect at least 1 row selected, start={}, end={}, step={}",
        start, end, step);
...
Tensor Tensor::select(int64_t dim, int64_t index) const {
    dim = wrap_size(dim, this->dim());
    HMP_REQUIRE(dim < this->dim(), "select : dim({}) is out of range {}", dim,
                this->dim());

    index = wrap_size(index, size(dim));
    HMP_REQUIRE(index < size(dim),
                "select: index {} is out of range, need less than {}", index,
                size(dim));
    ...
    out.tensorInfo()->setSizesAndStrides(newShape, newStrides,
...
Tensor arange(int64_t start, int64_t end, int64_t step,
    ...
    HMP_REQUIRE(start < end, "arange: expect start < end, got start={}, end={}",
                start, end);
...
#define DEFINE_TENSOR_BOPS(name, op)                                           \
    ...
    Tensor Tensor::operator op(const Tensor &b) const { return name(b); }      \
    Tensor &Tensor::operator op##=(const Tensor &b) { return name##_(b); }     \
    ...
    Tensor Tensor::operator op(const Scalar &b) const { return name(b); }      \
    Tensor &Tensor::operator op##=(const Scalar &b) { return name##_(b); }
...
Tensor operator*(const Scalar &a, const Tensor &b) { return b * a; }
...
// shape transform
Tensor Tensor::flatten() const { return reshape({-1}); }

void Tensor::tofile(const std::string &fn) { hmp::tofile(*this, fn); }
...
Tensor vstack(const TensorList &tensors) { return kernel::vstack(tensors); }
...
Tensor fromfile(const std::string &fn, ScalarType dtype, int64_t count,
                int64_t offset) {
...
void tofile(const Tensor &data, const std::string &fn) {
    ...
    auto nwrite =
        fwrite(tmp.unsafe_data(), tmp.itemsize(), tmp.nitems(), fp.get());
    HMP_REQUIRE(nwrite == tmp.nitems(),
                "write data to file failed, errno={} {}, {}", errno, nwrite,
                tmp.nitems());
}
