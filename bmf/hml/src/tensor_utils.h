...
struct ShapeStridePair {
    // Name type instead of a pair/tuple to make sure constuct the vectors
    // inplance
    // and get NRVO
    SizeArray shape;
    SizeArray strides;
...
inline SizeArray inferSize(const SizeArray shape, int64_t nitems) {
    ...
    if (numAny == 1) {
        HMP_REQUIRE(nitems % sizeWithOutAny == 0,
...
inline optional<SizeArray> computeStride(const SizeArray &oldShape,
    ...
    for (int64_t tensorD = oldShape.size() - 1; tensorD >= 0; tensorD--) {
        ...
        if ((tensorD == 0) ||
            (oldShape[tensorD - 1] != 1 &&
             oldStrides[tensorD - 1] != tensorItems * chunkStride)) {
            while (viewD >= 0 &&
...
inline ShapeStridePair inferExpandGeometry(const SizeArray &tensor_sizes,
...
ShapeStridePair inferUnsqueezeGeometry(const Tensor &tensor, int64_t dim) {
    ...
    int64_t new_stride =
        dim >= tensor.dim() ? 1 : result.shape[dim] * result.strides[dim];
...
void checkSizeArray(const SizeArray &sizes, const char *ctx) {
    ...
    for (size_t i = 0; i < sizes.size(); ++i) {
        HMP_REQUIRE(sizes[i] > 0, "Invalid size {} at dim {} detected in {}",
                    sizes[i], i, ctx);
    }
}
