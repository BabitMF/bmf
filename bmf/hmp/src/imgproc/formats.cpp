...
static ColorRange infer_color_range(PixelFormat fmt,
                                    ColorRange hint = CR_UNSPECIFIED) {
...
ColorModel::ColorModel()
    : ColorModel(CS_UNSPECIFIED, CR_UNSPECIFIED, CP_UNSPECIFIED,
                 CTC_UNSPECIFIED) {}

ColorModel::ColorModel(ColorSpace cs, ColorRange cr)
    : ColorModel(cs, cr, CP_UNSPECIFIED, CTC_UNSPECIFIED) {}

ColorModel::ColorModel(ColorPrimaries cp, ColorTransferCharacteristic ctc)
    : ColorModel(CS_UNSPECIFIED, CR_UNSPECIFIED, cp, ctc) {}

ColorModel::ColorModel(ColorSpace cs, ColorRange cr, ColorPrimaries cp,
                       ColorTransferCharacteristic ctc) {
    cm_ = cs | (cr << 8) | (cp << 16) | (ctc << 24);
}

ColorSpace ColorModel::space() const { return ColorSpace(cm_ & 0xff); }

ColorRange ColorModel::range() const { return ColorRange((cm_ >> 8) & 0xff); }
...
PixelInfo::PixelInfo() : format_(PF_NONE) {}
...
PixelInfo::PixelInfo(PixelFormat format, ColorPrimaries cp,
                     ColorTransferCharacteristic ctc, int align)
    : format_(format),
      color_model_(CS_UNSPECIFIED, infer_color_range(format), cp, ctc),
      align_(align) {}
...
std::string stringfy(const PixelInfo &pix_info) {
    return fmt::format("PixelInfo({}, {}, {}, {}, {})", pix_info.format(),
                       pix_info.space(), pix_info.range(), pix_info.primaries(),
                       pix_info.transfer_characteristic());
}
...
std::string stringfy(const ChannelFormat &format) {
    switch (format) {
    case ChannelFormat::NCHW:
        return "kNCHW";
    case ChannelFormat::NHWC:
        return "kNHWC";
    default:
...
struct PixelFormatDesc::Private {
    ...
    ScalarType dtype;          //
    int nplanes;               //
    int32_t ratio[sMaxPlanes]; // ratio related to first plane, packed like
                               // `(height_ratio << 8) | width_ratio<<4 |
                               // channels`
};
...
static PixelFormatDesc::Private sPixelFormatMetas[]{
    ...
    PixelFormatDesc::Private{
        PixelFormat::PF_YUV420P, kUInt8, 3, {0x111, 0x221, 0x221, 0x0}},
    PixelFormatDesc::Private{
        PixelFormat::PF_YUVJ420P, kUInt8, 3, {0x111, 0x221, 0x221, 0x0}},
    PixelFormatDesc::Private{
        PixelFormat::PF_YUV422P, kUInt8, 3, {0x111, 0x121, 0x121, 0x0}},
    PixelFormatDesc::Private{
        PixelFormat::PF_YUV444P, kUInt8, 3, {0x111, 0x111, 0x111, 0x0}},
    PixelFormatDesc::Private{
        PixelFormat::PF_NV12, kUInt8, 2, {0x111, 0x222, 0x00, 0x0}},
    PixelFormatDesc::Private{
        PixelFormat::PF_NV21, kUInt8, 2, {0x111, 0x222, 0x00, 0x0}},
    PixelFormatDesc::Private{
        PixelFormat::PF_P010LE, kUInt16, 2, {0x111, 0x222, 0x00, 0x0}}, //
    PixelFormatDesc::Private{
        PixelFormat::PF_P016LE, kUInt16, 2, {0x111, 0x222, 0x00, 0x0}}, //
    PixelFormatDesc::Private{
    ...
    PixelFormatDesc::Private{
        PixelFormat::PF_YUVA420P, kUInt8, 4, {0x111, 0x221, 0x221, 0x111}}, //
    PixelFormatDesc::Private{
        PixelFormat::PF_GRAY16, kUInt16, 1, {0x111, 0x00, 0x00, 0x0}}, //
    PixelFormatDesc::Private{
    ...
    PixelFormatDesc::Private{
        PixelFormat::PF_YA8, kUInt8, 1, {0x112, 0x00, 0x00, 0x0}}, //
    PixelFormatDesc::Private{
        PixelFormat::PF_RGBA32, kUInt8, 1, {0x114, 0x00, 0x00, 0x0}}, //
    PixelFormatDesc::Private{
        PixelFormat::PF_ARGB32, kUInt8, 1, {0x114, 0x00, 0x00, 0x0}}, //
    PixelFormatDesc::Private{
        PixelFormat::PF_ABGR32, kUInt8, 1, {0x114, 0x00, 0x00, 0x0}}, //
    PixelFormatDesc::Private{
...
PixelFormatDesc::PixelFormatDesc(int format) : pix_format_(format) {
...
int PixelFormatDesc::format() const { return pix_format_; }

int PixelFormatDesc::channels(int plane) const {
    HMP_REQUIRE(defined(), "PixelFormat {} is not supported", pix_format_);
    HMP_REQUIRE(plane < meta_->nplanes, "Plane index {} is out of range {}",
                plane, meta_->nplanes);
...
int PixelFormatDesc::infer_nitems(int width, int height, int plane) const {
    HMP_REQUIRE(defined(), "PixelFormat {} is not supported", pix_format_);
    return infer_width(width, plane) * infer_height(height, plane) *
           channels(plane);
}
