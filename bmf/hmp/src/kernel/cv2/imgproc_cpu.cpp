...
inline Tensor &img_morph_cpu(cv::MorphTypes algo, Tensor &dst,
                             const Tensor &src, const Tensor &kernel,
                             ChannelFormat cformat) {
    auto kmat = ocv::to_cv_mat(kernel, false);
    ocv::foreach_image(
        [&](cv::Mat &dmat, const cv::Mat &smat) {
...
Tensor &img_sobel_cpu(Tensor &dst, const Tensor &src, int64_t dx, int64_t dy,
    ...
    ocv::foreach_image(
        [&](cv::Mat &dmat, const cv::Mat &smat) {
            ...
            cv::Sobel(smat, dmat, ddepth, dx, dy, ksize, scale.to<double>(),
                      delta.to<double>());
        },
        cformat, dst, src);
...
Tensor &img_canny_cpu(Tensor &dst, const Tensor &src, const Scalar &threshold1,
                      const Scalar &threshold2, int64_t aperture_size,
    ...
    ocv::foreach_image(
        [&](cv::Mat &dmat, const cv::Mat &smat) {
            cv::Canny(smat, dmat, threshold1.to<double>(),
                      threshold2.to<double>(), aperture_size, l2_gradient);
        },
        cformat, dtmp, stmp);
...
Tensor &img_filter2d_cpu(Tensor &dst, const Tensor &src, const Tensor &kernel,
                         const Scalar &delta, ChannelFormat cformat) {
    auto kmat = ocv::to_cv_mat(kernel, false);
    ocv::foreach_image(
        [&](cv::Mat &dmat, const cv::Mat &smat) {
            auto ddepth = ocv::to_cv_type(dst.dtype());
            cv::filter2D(smat, dmat, ddepth, kmat, cv::Point(-1, -1),
                         delta.to<double>());
        },
        cformat, dst, src);
...
Tensor &img_gaussian_blur_cpu(Tensor &dst, const Tensor &src, int kx, int ky,
    ...
    ocv::foreach_image(
        [&](cv::Mat &dmat, const cv::Mat &smat) {
            cv::GaussianBlur(smat, dmat, {kx, ky}, sigma_x.to<double>(),
                             sigma_y.to<double>());
        },
        cformat, dst, src);
...
Tensor &img_bilateral_filter_cpu(Tensor &dst, const Tensor &src, int d,
    ...
    ocv::foreach_image(
        [&](cv::Mat &dmat, const cv::Mat &smat) {
            cv::bilateralFilter(smat, dmat, d, sigma_color.to<double>(),
                                sigma_space.to<double>());
        },
        kNHWC, dtmp, stmp);
...
Tensor &img_warp_perspective_cpu(Tensor &dst, const Tensor &src,
    ...
    ocv::foreach_image(
        [&](cv::Mat &dmat, const cv::Mat &smat) {
            ...
            cv::warpPerspective(smat, dmat, m, dsize, flags,
                                cv::BORDER_REPLICATE);
        },
        cformat, dst, src);
