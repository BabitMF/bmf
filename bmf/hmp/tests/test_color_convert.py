...
import numpy as np
import os
from hmp_fixtures import device_type
from hmp_fixtures import mp, TEST_DATA_ROOT
import glob
...
def read_yuv(fn, width, height, fmt):
    ...
    if fmt[-3:] == "420":
        ...
        i += width * height
        u = yuv[i:i + width * height // 4].reshape(
            (height // 2, width // 2, 1))
        i += width * height // 4
        v = yuv[i:i + width * height // 4].reshape(
            (height // 2, width // 2, 1))
        yuv = [y, u, v]
    elif fmt[-3:] == "422":
        i = 0
        ...
        i += width * height
        uv = yuv[i:i + width * height // 2].reshape(
            (height // 2, width // 2, 2))
        yuv = [y, uv]
    else:
        assert (False)
...
@pytest.fixture(
    params=[pytest.param(p, id=str(p[0])) for p in get_yuv_rgb_pairs()],
    scope='session')
def yuv_rgb(request):
...
def test_yuv_rgb_convert(yuv_rgb, device_type):
    ...
    v_rgb = mp.img.yuv_to_rgb(im_yuv0,
                              pix_info=infer_pix_info(format),
                              cformat=mp.kNHWC)
    v_yuv = mp.img.rgb_to_yuv(im_rgb,
                              pix_info=infer_pix_info(format),
                              cformat=mp.kNHWC)
