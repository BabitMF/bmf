...
import os
import cv2
from hmp_fixtures import device_type, img_dtype, channel_format, channels
from hmp_fixtures import mp, lenna_image, to_np_dtype
import glob
...
class TestImageResize(object):
    ...
    @pytest.fixture(params=[
        pytest.param(m, id=repr(m))
        for m in (mp.kNearest, mp.kBilinear, mp.kBicubic)
    ])
    ...
    def test_image_resize(self, lenna_image, mode, device_type, img_dtype,
                          channel_format):
        format = channel_format

        origin, mp_origin = pre_process_image(lenna_image, device_type,
                                              channel_format, img_dtype)

        sizes = [(240, 160), (320, 240), (480, 360), (1280, 720), (1920, 1080),
                 (3840, 2160)]

        for size in sizes:
            ref = cv2.resize(origin,
                             size,
                             interpolation=cv2_resize_interp(mode))
            mp_out = mp.img.resize(mp_origin,
                                   size[0],
                                   size[1],
                                   mode=mode,
                                   format=format)
    ...
    def test_image_resize_channels(self, lenna_image, device_type,
...
class TestImageTransform(object):
    ...
    @pytest.fixture(params=[
        pytest.param(m, id=repr(m[0]))
        for m in ((mp.kRotate0, -1), (mp.kRotate90, cv2.ROTATE_90_CLOCKWISE),
    ...
    def test_rotate(self, lenna_image, rotate_mode, device_type, img_dtype,
                    channel_format):
    ...
    @pytest.fixture(params=[
        pytest.param(m, id=repr(m))
        for m in (mp.kVertical, mp.kHorizontal, mp.kHorizontalAndVertical)
    ])
    def axes(self, request):
        return request.param

    def test_mirror(self, lenna_image, axes, device_type, img_dtype,
                    channel_format):
        lenna_image = cv2.resize(lenna_image, (1280, 720))
        origin, mp_origin = pre_process_image(lenna_image, device_type,
                                              channel_format, img_dtype)
    ...
    def test_normalize(self, lenna_image, device_type, img_dtype,
                       channel_format):
        ...
        std = np.array([255 / 2, 255 / 2.1, 255 / 2.2],
        ...
        mp_out = mp.img.normalize(mp_origin,
                                  mp_mean,
                                  mp_std,
                                  format=channel_format)
