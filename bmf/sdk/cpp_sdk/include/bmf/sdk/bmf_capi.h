...
////////// AudioFrame ////////////
BMF_API bmf_AudioFrame bmf_af_make_from_data(hmp_Tensor *data, int size, uint64_t layout, bool planer);
BMF_API bmf_AudioFrame bmf_af_make(int samples, uint64_t layout, bool planer, int dtype);
...
BMF_API const bmf_JsonParam bmf_af_private_get_json_param(const bmf_AudioFrame af);
BMF_API void bmf_af_private_attach_json_param(bmf_AudioFrame af, const bmf_JsonParam json_param);
...
BMF_API void bmf_pkt_copy_props(bmf_BMFAVPacket pkt, const bmf_BMFAVPacket from);
BMF_API void bmf_pkt_private_merge(bmf_BMFAVPacket pkt, const bmf_BMFAVPacket from);
BMF_API const bmf_JsonParam bmf_pkt_private_get_json_param(const bmf_BMFAVPacket pkt);
BMF_API void bmf_pkt_private_attach_json_param(bmf_BMFAVPacket pkt, const bmf_JsonParam json_param);
...
BMF_API bmf_Packet bmf_packet_from_bmfavpacket(const bmf_BMFAVPacket bmf_av_pkt);
...
////////// Task ////////////
BMF_API bmf_Task bmf_task_make(int node_id, int *istream_ids, int ninputs, int *ostream_ids, int noutputs);
BMF_API void bmf_task_free(bmf_Task task);
BMF_API int bmf_task_fill_input_packet(bmf_Task task, int stream_id, const bmf_Packet packet);
BMF_API int bmf_task_fill_output_packet(bmf_Task task, int stream_id, const bmf_Packet packet);
BMF_API bmf_Packet bmf_task_pop_packet_from_out_queue(bmf_Task task, int stream_id);
BMF_API bmf_Packet bmf_task_pop_packet_from_input_queue(bmf_Task task, int stream_id);
...
BMF_API int bmf_module_functor_execute(bmf_ModuleFunctor mf, const bmf_Packet *inputs, int ninputs, bool cleanup, bool *is_done);
BMF_API bmf_Packet* bmf_module_functor_fetch(bmf_ModuleFunctor mf, int index, int *noutputs, bool *is_done);
