...
template<>
struct OpaqueDataInfo<AVPacket>
{
...
/**
 * @brief Convert VideoFrame to AVFrame, if AVFrame have been attach to this VideoFrame, 
 * it will be used as a prototype
 * 
 * @param vf 
 * @param force_private if true, AVFrame must exists in VideoFrame as private data
 * @return AVFrame* 
 */
static AVFrame *from_video_frame(const VideoFrame &vf, bool force_private = true)
{
    auto avf_ref = vf.private_get<AVFrame>();
    if(!avf_ref && force_private){
        throw std::runtime_error("No AVFrame found in VideoFrame as private_data");
    }
    ...
    std::string s_tb = std::to_string(vf.time_base().num) + "," + std::to_string(vf.time_base().den);
    ...
    if(avf->hw_frames_ctx){
        ...
        // we can't do that in this func, as vf does not actually own vf->stream()
    }
...
/**
 * @brief Convert AVFrame to VideoFrame, VideoFrame share the same buffer with AVFrame
 * 
 * @param avf 
...
static VideoFrame to_video_frame(const AVFrame *avf, bool attach = true)
{
    ...
    //
    vf.set_stream(hmp::ffmpeg::av_hw_frames_ctx_to_stream(
            avf->hw_frames_ctx, "to_video_frame"));

    return vf;
}


static AVFrame *from_audio_frame(const AudioFrame &af, bool force_private = true)
{
    auto aaf_ref = af.private_get<AVFrame>();
    if(!aaf_ref && force_private){
        throw std::runtime_error("No AVFrame found in AudioFrame as private_data");
    }
    ...
    //FIXME: remove after encoder&decoder update
    std::string s_tb = std::to_string(af.time_base().num) + "," + std::to_string(af.time_base().den);
...
/**
 * @brief Convert BMFAVPacket to AVPacket, if force_private is true, AVPacket must 
 ...
 * @return AVPacket* 
 */
static AVPacket* from_bmf_av_packet(const BMFAVPacket &pkt, bool force_private = true)
{
    auto avp_ref = pkt.private_get<AVPacket>();
    if(!avp_ref && force_private){
        throw std::runtime_error("No AVPacket found in BMFAVPacket as private_data");
    }
...
static BMFAVPacket to_bmf_av_packet(const AVPacket *avp_, bool attach = true)
{
    ...
    auto data_ptr = DataPtr(avp->data, [=](void*){
        av_packet_free((AVPacket**)&avp);
    }, kCPU);
...
static AVFramePtr make_avframe(int width, int height, int format, int align = 32)
{
...
static VideoFrame reformat(const VideoFrame &vf, AVPixelFormat format, void **context = nullptr)
{
    ...
    if(vf.is_image()){
        ...
        auto tmp = vf.image().format() == kNCHW ? vf.image().to(kNHWC, true) : vf.image();

        // allocate dest AVFrame
        dst_frame = make_avframe(vf.width(), vf.height(), format);

        sws_context = sws_getCachedContext(cached_context, vf.width(), vf.height(), (AVPixelFormat)avformat,
                                                dst_frame->width, dst_frame->height, format,
                                                SWS_BILINEAR, NULL, NULL, NULL);

        uint8_t *src_data[4] = {(uint8_t*)tmp.data().unsafe_data(), 0, 0, 0};
        int src_linesize[4] = {int(tmp.data().stride(0) * tmp.data().itemsize()), 0, 0, 0};
        int height = sws_scale(sws_context, src_data, src_linesize, 0, vf.height(),
        ...
        sws_context = sws_getCachedContext(cached_context, vf.width(), vf.height(), (AVPixelFormat)src_frame->format,
                                                dst_frame->width, dst_frame->height, format,
                                                SWS_BILINEAR, NULL, NULL, NULL);
        int height = sws_scale(sws_context, src_frame->data, src_frame->linesize, 0, vf.height(),
                               dst_frame->data, dst_frame->linesize);
        HMP_REQUIRE(height == vf.height(), "ffmpeg::reformat internal error");
    }
...
static VideoFrame reformat(const VideoFrame &vf, const std::string &format_str, void **context = nullptr)
{
