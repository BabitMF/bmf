...
BEGIN_BMF_SDK_NS
    ...
    public:
        ...
        static void
        AddConstructor(std::string const &module_name, std::string const &sdk_version, Constructor constructor);

        static std::shared_ptr<Module>
        ConstructModule(std::string const &module_name, int node_id = -1, JsonParam json_param = JsonParam());
    ...
    public:
        ModuleRegister(std::string const &module_name, std::string const &sdk_version,
...
#define BMF_IMPORT_MODULE(module_name) HMP_IMPORT_TAG(r_constructor_##module_name);
