...
/* TraceUserInfo is an unordered map to store user info as string
    Note that the key or value should not contain ":" as it is a reserved character
...
inline int trace_get_buffer_size() {
    if (getenv("BMF_TRACE_BUFFER_SIZE")) {
        return strtoll(
            getenv("BMF_TRACE_BUFFER_SIZE"), NULL, 10
        );
    }
...
/* Time interval (in clock cycles) between batched logs
    This ensures logs to be tranched instead of continuous especially for long operation
*/
inline int64_t TRACE_BINLOG_INTERVAL = 1000000 * 10;    // 10M clock cycles per binary log
...
*/
    ...
    */
    ...
    public:
        ...
        static TraceLogger *instance() {
            if (!traceLogger) {
                ...
                if (getenv("BMF_TRACE_BUFFER_COUNT")) {
                    thread_count = strtoll(
                        getenv("BMF_TRACE_BUFFER_COUNT"), NULL, 10
                    );
                }
        ...
        // Notes a closing thread, hence reducing the current number of running threads
    ...
    */
    ...
    inline void BMF_TRACE_QUEUE_INFO(
        const char *name,
        int queue_size,
        int max_size,
        const char* src = __builtin_FUNCTION()
    ...
    inline void BMF_TRACE_THROUGHPUT(
        int stream_id,
        int node_id,
        int queue_size
    ) {
        if (TRACE_ALLOWED_TYPES >> THROUGHPUT & 1) {
            ...
            threadTracer.trace_info(
                ...
                NONE, info.get(), 
                ("Node" + std::to_string(node_id)).c_str()
            );
        }
    }
    ...
    public:
        ...
        TraceProcessEmitter(TraceType category, std::string node_name, const char* src = __builtin_FUNCTION()) 
