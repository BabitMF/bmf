...
struct AudioFrame::Private {
    ...
    static TensorList construct(int samples, uint64_t layout, bool planer,
                                const TensorOptions &options) {
        Tensor data_s =
            hmp::empty(infer_shape(samples, planer, layout), options);
        auto channels = popcount64_c(layout);
...
AudioFrame::AudioFrame(const TensorList &data, uint64_t layout, bool planer) {
    ...
    if (planer) {
        HMP_REQUIRE(channels == 0 || data.size() == channels,
                    "AudioFrame: data shape does not match channel layout, "
                    "expect channels {}, got {}",
                    channels, data.size());
        for (auto &d : data) {
...
AudioFrame::AudioFrame(int samples, uint64_t layout, bool planer,
                       const TensorOptions &options)
    : AudioFrame(Private::construct(samples, layout, planer, options), layout,
                 planer) {}
...
AudioFrame::operator bool() const { return self.get() != nullptr; }

uint64_t AudioFrame::layout() const { return self->layout; }

bool AudioFrame::planer() const { return self->planer; }

int AudioFrame::nsamples() const { return self->data[0].size(0); }
...
ScalarType AudioFrame::dtype() const { return self->data[0].dtype(); }

void AudioFrame::set_sample_rate(float sample_rate) {
    HMP_REQUIRE(sample_rate > 0, "AudioFrame: expect sample_rate > 0, got {}",
                sample_rate);
    self->sample_rate = sample_rate;
}

float AudioFrame::sample_rate() const { return self->sample_rate; }

const TensorList &AudioFrame::planes() const { return self->data; }

int AudioFrame::nplanes() const { return self->data.size(); }
...
Tensor AudioFrame::operator[](int p) const { return plane(p); }
