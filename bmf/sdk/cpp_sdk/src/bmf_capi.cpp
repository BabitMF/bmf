...
bmf_JsonParam bmf_json_param_parse(const char *str) {
    BMF_PROTECT(bmf_JsonParam j = new JsonParam(); j->parse(str); return j;)
    return nullptr;
}
...
bmf_VideoFrame bmf_vf_from_frame(hmp_Frame frame) {
    BMF_PROTECT(return new VideoFrame(*frame);)
...
bool bmf_vf_defined(const bmf_VideoFrame vf) { return *vf; }
int bmf_vf_width(const bmf_VideoFrame vf) { return vf->width(); }

int bmf_vf_height(const bmf_VideoFrame vf) { return vf->height(); }

int bmf_vf_dtype(const bmf_VideoFrame vf) { return (int)vf->dtype(); }

const hmp_Frame bmf_vf_frame(const bmf_VideoFrame vf) {
    BMF_PROTECT(return (const hmp_Frame)&vf->frame();)

    return nullptr;
}

bmf_VideoFrame bmf_vf_cpu(const bmf_VideoFrame vf, bool non_blocking) {
    BMF_PROTECT(auto tmp = vf->cpu(non_blocking); return new VideoFrame(tmp);)
    return nullptr;
}

bmf_VideoFrame bmf_vf_cuda(const bmf_VideoFrame vf) {
    BMF_PROTECT(auto tmp = vf->cuda(); return new VideoFrame(tmp);)
    return nullptr;
}
...
bmf_VideoFrame bmf_vf_to_device(const bmf_VideoFrame vf, const char *device,
                                bool non_blocking) {
    BMF_PROTECT(return new VideoFrame(vf->to(Device(device), non_blocking));)
    return nullptr;
}
...
void bmf_vf_private_attach_json_param(bmf_VideoFrame vf,
                                      const bmf_JsonParam json_param) {
    vf->private_attach<JsonParam>(json_param);
}

bmf_VideoFrame bmf_vf_reformat(const bmf_VideoFrame vf,
                               const hmp_PixelInfo pix_info) {
    BMF_PROTECT(return new VideoFrame(vf->reformat(*pix_info));)
    return nullptr;
}

void bmf_vf_set_pts(bmf_VideoFrame vf, int64_t pts) { vf->set_pts(pts); }

int64_t bmf_vf_pts(bmf_VideoFrame vf) { return vf->pts(); }
...
bool bmf_vf_ready(const bmf_VideoFrame vf) { return vf->ready(); }

void bmf_vf_record(bmf_VideoFrame vf, bool use_current) {
    vf->record(use_current);
}

void bmf_vf_synchronize(bmf_VideoFrame vf) { vf->synchronize(); }

////////// AudioFrame ////////////
bmf_AudioFrame bmf_af_make_from_data(hmp_Tensor *data, int size,
                                     uint64_t layout, bool planer) {
    BMF_PROTECT(std::vector<Tensor> tensor_list;
...
bmf_AudioFrame bmf_af_make(int samples, uint64_t layout, bool planer,
                           int dtype) {
    BMF_PROTECT(auto tensor_options =
                    hmp::TensorOptions((hmp::ScalarType)dtype);
...
bool bmf_af_defined(const bmf_AudioFrame af) { return *af; }

uint64_t bmf_af_layout(const bmf_AudioFrame af) { return af->layout(); }

int bmf_af_dtype(const bmf_AudioFrame af) { return (int)af->dtype(); }

bool bmf_af_planer(const bmf_AudioFrame af) { return af->planer(); }

int bmf_af_nsamples(const bmf_AudioFrame af) { return af->nsamples(); }

int bmf_af_nchannels(const bmf_AudioFrame af) { return af->nchannels(); }

void bmf_af_set_sample_rate(const bmf_AudioFrame af, float sr) {
    af->set_sample_rate(sr);
}

float bmf_af_sample_rate(const bmf_AudioFrame af) { return af->sample_rate(); }
...
int bmf_af_nplanes(const bmf_AudioFrame af) { return af->nplanes(); }

hmp_Tensor bmf_af_plane(const bmf_AudioFrame af, int p) {
    BMF_PROTECT(return new Tensor(af->plane(p));)
    return nullptr;
}
...
void bmf_af_private_attach_json_param(bmf_AudioFrame af,
                                      const bmf_JsonParam json_param) {
    af->private_attach<JsonParam>(json_param);
}

void bmf_af_set_pts(bmf_AudioFrame af, int64_t pts) { af->set_pts(pts); }

int64_t bmf_af_pts(bmf_AudioFrame af) { return af->pts(); }
...
bmf_BMFAVPacket bmf_pkt_make_from_data(hmp_Tensor data) {
    BMF_PROTECT(return new BMFAVPacket(*data);)
    return nullptr;
}
...
bool bmf_pkt_defined(const bmf_BMFAVPacket pkt) { return *pkt; }

hmp_Tensor bmf_pkt_data(const bmf_BMFAVPacket pkt) {
    BMF_PROTECT(return new Tensor(pkt->data());)
    return nullptr;
}

void *bmf_pkt_data_ptr(bmf_BMFAVPacket pkt) { return pkt->data_ptr(); }

const void *bmf_pkt_data_const_ptr(const bmf_BMFAVPacket pkt) {
    return pkt->data_ptr();
}

int bmf_pkt_nbytes(const bmf_BMFAVPacket pkt) { return pkt->nbytes(); }
...
void bmf_pkt_private_attach_json_param(bmf_BMFAVPacket pkt,
                                       const bmf_JsonParam json_param) {
    pkt->private_attach<JsonParam>(json_param);
}

void bmf_pkt_set_pts(bmf_BMFAVPacket pkt, int64_t pts) { pkt->set_pts(pts); }

int64_t bmf_pkt_pts(bmf_BMFAVPacket pkt) { return pkt->pts(); }
...
int64_t bmf_pkt_offset(bmf_BMFAVPacket pkt) { return pkt->get_offset(); }

int64_t bmf_pkt_whence(bmf_BMFAVPacket pkt) { return pkt->get_whence(); }
...
bmf_Packet bmf_packet_generate_empty_packet() { return new Packet(); }

int64_t bmf_packet_timestamp(const bmf_Packet pkt) { return pkt->timestamp(); }
...
bmf_Packet bmf_packet_from_videoframe(const bmf_VideoFrame vf) {
    BMF_PROTECT(return new Packet(*vf);)

    return nullptr;
}

bmf_VideoFrame bmf_packet_get_videoframe(const bmf_Packet pkt) {
    BMF_PROTECT(return new VideoFrame(pkt->get<VideoFrame>());)
    return nullptr;
}
...
bmf_Packet bmf_packet_from_audioframe(const bmf_AudioFrame af) {
    BMF_PROTECT(return new Packet(*af);)

    return nullptr;
}

bmf_AudioFrame bmf_packet_get_audioframe(const bmf_Packet pkt) {
    BMF_PROTECT(return new AudioFrame(pkt->get<AudioFrame>());)
    return nullptr;
}
...
bmf_Packet bmf_packet_from_bmfavpacket(const bmf_BMFAVPacket bmf_av_pkt) {
    BMF_PROTECT(return new Packet(*bmf_av_pkt);)

    return nullptr;
}

bmf_BMFAVPacket bmf_packet_get_bmfavpacket(const bmf_Packet pkt) {
    BMF_PROTECT(return new BMFAVPacket(pkt->get<BMFAVPacket>());)
    return nullptr;
}
...
bmf_Packet bmf_packet_from_json_param(const bmf_JsonParam json) {
    BMF_PROTECT(return new Packet(*json);)
    return nullptr;
}

bmf_JsonParam bmf_packet_get_json_param(const bmf_Packet pkt) {
    BMF_PROTECT(return new JsonParam(pkt->get<JsonParam>());)
    return nullptr;
}

bmf_Packet bmf_packet_from_string_param(char *const str) {
    BMF_PROTECT(std::string string_value(str); return new Packet(string_value);)
    return nullptr;
}

char *const bmf_packet_get_string_param(const bmf_Packet pkt) {
    BMF_PROTECT(return bmf_strdup(pkt->get<std::string>().c_str());)
    return nullptr;
}
...
//////////////////// Task /////////////////////
bmf_Task bmf_task_make(int node_id, int *istream_ids, int ninputs,
                       int *ostream_ids, int noutputs) {
    BMF_PROTECT(std::vector<int> iids(istream_ids, istream_ids + ninputs);
...
int bmf_task_fill_input_packet(bmf_Task task, int stream_id,
                               const bmf_Packet packet) {
    BMF_PROTECT(return task->fill_input_packet(stream_id, *packet);)
    return 0;
}

int bmf_task_fill_output_packet(bmf_Task task, int stream_id,
                                const bmf_Packet packet) {
    BMF_PROTECT(return task->fill_output_packet(stream_id, *packet);)
    return 0;
}
...
int64_t bmf_task_timestamp(const bmf_Task task) { return task->timestamp(); }
...
int bmf_task_get_input_stream_ids(bmf_Task task, int *ids) {
    BMF_PROTECT(auto sids = task->get_input_stream_ids();
                for (size_t i = 0; i < sids.size() && ids;
                     ++i) { ids[i] = sids[i]; } return sids.size();)
    return -1;
}

int bmf_task_get_output_stream_ids(bmf_Task task, int *ids) {
    BMF_PROTECT(auto sids = task->get_output_stream_ids();
                for (size_t i = 0; i < sids.size() && ids;
                     ++i) { ids[i] = sids[i]; } return sids.size();)
    return -1;
}

int bmf_task_get_node(bmf_Task task) { return task->get_node(); }

////////// ModuleTag ///////////
bmf_ModuleTag bmf_module_tag_make(int64_t tag) {
    BMF_PROTECT(return new ModuleTag(static_cast<ModuleTag>(tag));)
    return nullptr;
}
...
bmf_ModuleInfo bmf_module_info_make() {
    BMF_PROTECT(return new ModuleInfo();)
    return nullptr;
}
...
void bmf_module_info_set_description(bmf_ModuleInfo info,
...
//////////////// ModuleFunctor ////////////
bmf_ModuleFunctor bmf_module_functor_make(const char *name, const char *type,
                                          const char *path, const char *entry,
                                          const char *option, int ninputs,
                                          int noutputs, int node_id) {
    BMF_PROTECT(auto &M = ModuleManager::instance();
                ModuleInfo info(name, type, entry, path);
                auto factory = M.load_module(info); if (factory == nullptr) {
                    throw std::runtime_error("Load module " + info.module_name +
                                             " failed");
