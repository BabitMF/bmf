...
ModuleFunctor::ModuleFunctor(const std::shared_ptr<Module> &m, int ninputs,
                             int noutputs) {
...
ModuleFunctor &ModuleFunctor::execute(const std::vector<Packet> &inputs,
                                      bool cleanup) {
    HMP_REQUIRE(inputs.size() == self->iids.size(),
                "Expect {} inputs, got {} inputs", self->iids.size(),
                inputs.size());
    ...
    if (rc != 0) {
        throw std::runtime_error(
            fmt::format("Process failed with error {}", rc));
    }
...
Module &ModuleFunctor::module() const { return *self->module; }

int ModuleFunctor::ninputs() const { return self->iids.size(); }

int ModuleFunctor::noutputs() const { return self->oids.size(); }

ModuleFunctor make_sync_func(const ModuleInfo &info, int32_t ninputs,
                             int32_t noutputs, const JsonParam &option,
                             int32_t node_id) {
