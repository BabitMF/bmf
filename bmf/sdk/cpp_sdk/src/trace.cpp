...
void TraceBuffer::push_event(const TraceEvent &event) {
    ...
    next_write_index_.store(get_next_index(next_write_index_),
...
TraceEvent TraceBuffer::pop_event() {
    ...
    next_read_index_.store(get_next_index(next_read_index_),
                           std::memory_order_relaxed);
    buffer_occupancy_--;
    return event;
}
...
int TraceBuffer::overflowed() const { return overflow_count_; }
...
TraceLogger::TraceLogger(int queue_size, bool loop_mode)
    ...
    if (getenv("BMF_TRACE_PRINTING") &&
        std::strcmp(getenv("BMF_TRACE_PRINTING"), "DISABLE") == 0)
        enable_printing = false;
    if (getenv("BMF_TRACE_LOGGING") &&
        std::strcmp(getenv("BMF_TRACE_LOGGING"), "DISABLE") == 0)
...
void TraceLogger::create_log() {
    ofs_.open(get_log_name(log_index_),
    ...
    for (int i = 0; i < queue_map_.size(); i++) {
        while (!queue_map_[i].is_empty()) {
            ...
            ofs_ << queue_map_[i].process_name << ","
                 << queue_map_[i].thread_name << "," << event.timestamp << ","
                 << event.name << ":" << event.subname << "," << event.category
                 << "," << event.phase << event.info << std::endl;
        }
    }
...
void ThreadTrace::trace(TraceType category, const char *name, TracePhase phase,
                        const char *src) {
    // Get the current timestamp
    int64_t timestamp = BMF_TRACE_CLOCK() - BMF_TRACE_CLOCK_START;

    // Create a trace event
    TraceEvent event = TraceEvent(timestamp, name, src, category, phase);

    // Send the event to buffer
    TraceLogger::instance()->push(thread_id_, event);
}

void ThreadTrace::trace_info(TraceType category, const char *name,
                             TracePhase phase, std::string info,
                             const char *src) {
    // Get the current timestamp
    int64_t timestamp = BMF_TRACE_CLOCK() - BMF_TRACE_CLOCK_START;

    // Create a trace event
    TraceEvent event = TraceEvent(timestamp, name, src, category, phase, info);

    // Send the event to buffer
    TraceLogger::instance()->push(thread_id_, event);
}

void ThreadTrace::trace_process(const char *name, const char *subname,
                                TracePhase phase) {
    // Get the current timestamp
    int64_t timestamp = BMF_TRACE_CLOCK() - BMF_TRACE_CLOCK_START;

    // Create a trace event
    TraceEvent event = TraceEvent(timestamp, name, subname, PROCESSING, phase);
...
void TraceLogger::format_logs(bool include_info) {
    ...
    std::map<std::string, std::map<std::string, std::pair<long long, uint16_t>>>
        throughput;
...
// Proceed if is a log file
            ...
            while (fgets(line, sizeof(line), fp)) {
                ...
                while (ss.good()) {
                    ...
                    if (term_count == 0) {
                        ...
                        // Phase
                        linelog["ph"] = phases[strtoll(term.c_str(), NULL, 10)];
                        ...
                        if (linelog["ph"] == "B") {
                            ...
                            if (enable_printing) {
                                ...
                                if (!throughput.count(node_name)) {
                                    throughput[node_name] = std::map<
                                        std::string,
                                        std::pair<long long, uint16_t>>();
                                }
                            }
                        } else if (linelog["ph"] == "E") {
                            // Duration event with phase 'E' will be checked
                            // against 'E'
                            if (previous_ts.count(linelog["name"])) {
                                long long last_ts =
                                    previous_ts[linelog["name"]];
                                // Assume 'B' only occur once, while 'E' can
                                // occur multiple times
                                if (ts_completed.count(linelog["name"]) &&
                                    ts_completed[linelog["name"]].count(
                                        last_ts)) {
                                    ...
                                    ts_completed[linelog["name"]][last_ts] =
                                        true;
                                }
                                ...
                                if (enable_printing && cat_index > 0) {
                                    ...
                                    timings[linelog["name"]]["total"] +=
                                        duration;
                                    timings[linelog["name"]]["count"]++;
                                    timings[linelog["name"]]["ave"] =
                                        timings[linelog["name"]]["total"] /
                                        timings[linelog["name"]]["count"];
                                    timings[linelog["name"]]["min"] = std::min(
                                        timings[linelog["name"]]["min"],
                                        duration);
                                    timings[linelog["name"]]["max"] = std::max(
                                    ...
                                    if (cat_index == 1) {
                                        // Handle throughput event - only end of
                ...
                if (enable_printing) {
                    if (cat_index == 3) {
                        ...
                        if (linelog["args"]["size"] >
                            queue_info[queue_name]["max"])
                            queue_info[queue_name]["max"] =
                                linelog["args"]["size"];
    ...
    std::sort(flog.begin(), flog.end(),
              [](const json_it_type a, const json_it_type b) {
                  return a["ts"] < b["ts"];
              });
    ...
    if (enable_printing) {
        ...
        std::cout << "\n\n- - - - - - - - - - - - EVENTS DURATION - - - - - - "
                  ...
                  << std::endl
                  << std::left << std::setw(30) << "Event Name" << std::setw(10)
                  << "Total (us)" << std::setw(10) << "Total (%)"
                  << std::setw(10) << "Ave (us)" << std::setw(10) << "Min (us)"
                  << std::setw(10) << "Max (us)";
        for (auto &it : timings) {
            float perc = it.second["total"] * 100 / trace_diff;
            std::cout << std::endl
                      << std::left << std::setw(30) << it.first << std::setw(10)
                      << it.second["total"] << std::setw(10) << std::fixed
                      << std::setprecision(1) << perc << std::setw(10)
                      << it.second["ave"] << std::setw(10) << it.second["min"]
        ...
        std::cout << "\n\n- - - - - - - - - - - - QUEUE INFORMATION - - - - - "
                     "- - - - - - -"
                  << std::endl
                  << std::left << std::setw(30) << "Queue Name" << std::setw(10)
                  << "Limit" << std::setw(10) << "Ave" << std::setw(10)
                  << "Max";

        for (auto &it : queue_info) {
            std::cout << std::endl
                      << std::left << std::setw(30) << it.first << std::setw(10)
                      << it.second["limit"] << std::setw(10) << it.second["ave"]
                      << std::setw(10) << it.second["max"];
        }
        ...
        std::cout << "\n\n- - - - - - - - - - - THROUGHPUT INFORMATION - - - - "
                     "- - - - - - -"
                  << std::endl
                  << std::left << std::setw(10) << "Node" << std::setw(10)
                  << "Stream" << std::setw(10) << "Packets/second";

        for (auto &node : throughput) {
            for (auto &stream : throughput[node.first]) {
                auto stream_rate =
                    stream.second.second * 1000000 / stream.second.first;
                std::cout << std::endl
                          << std::left << std::setw(30) << node.first
                          << std::setw(10) << stream.first << std::setw(10)
                          << stream_rate;
            }
        }
    ...
    if (include_info) {
        ...
        if (enable_printing) {
            std::cout << "\n\nTotal number of thread buffers allocated: "
                      << buffer_allocated << " / "
                      << logstats["args"]["buffer_count"]
                      << "\nAllocated buffer size (individual): "
                      << logstats["args"]["buffer_size"];
        }
    ...
    time_t now = time(0);
    strftime(tracelog_name, sizeof(tracelog_name), TRACELOG_NAME_FORMAT,
             localtime(&now));
