...
MediaInfo::MediaInfo(std::string filepath) {
    ...
    char buffer[128];
    std::string result = "";
    std::string cmd = "ffprobe -hide_banner -loglevel quiet -print_format json "
...
bool MediaInfo::MediaCompareEquals(std::string expected) {
            ...
            if (!extraInfo.is_null() && !extraInfo["fps"].is_null()) {
                ...
                if (fps_comps.size() == 1)
                    ...
                    throw std::runtime_error(
                        "Invalid fraction in expected media output");
                ...
                float expected_fps =
                    std::stof(extraInfo["fps"].get<std::string>());
    ...
    float actual_diff_duration =
        std::abs(std::stof(mediaJson["format"]["duration"].get<std::string>()) -
                 expected_duration);
    if (actual_diff_duration > expected_diff_duration) {
        std::cout << "Invalid duration: " << mediaJson["format"]["duration"]
                  << " != " << expected_comps[3] << std::endl;
        return false;
    }

    std::string format_name =
        mediaJson["format"]["format_name"].get<std::string>();
    std::string expected_format_name = expected_comps[4];
    std::transform(format_name.begin(), format_name.end(), format_name.begin(),
                   ::toupper);
    std::transform(expected_format_name.begin(), expected_format_name.end(),
                   expected_format_name.begin(), ::toupper);
    if (format_name != expected_format_name) {
        std::cout << "Invalid format: " << mediaJson["format"]["format_name"]
                  << " != " << expected_comps[4] << std::endl;
        return false;
    }

    float expected_bitrate = std::stof(expected_comps[5]);
    float expected_diff_bitrate = expected_bitrate * 0.2;
    float actual_diff_bitrate =
        std::abs(std::stof(mediaJson["format"]["bit_rate"].get<std::string>()) -
                 expected_bitrate);
    if (actual_diff_bitrate > expected_diff_bitrate) {
        std::cout << "Invalid bitrate: " << mediaJson["format"]["bit_rate"]
                  << " != " << expected_comps[5] << std::endl;
        return false;
    }
    ...
    float actual_diff_size =
        std::abs(std::stof(mediaJson["format"]["size"].get<std::string>()) -
                 expected_size);
    if (actual_diff_size > expected_diff_size) {
        std::cout << "Invalid size: " << mediaJson["format"]["size"]
                  << " != " << expected_comps[6] << std::endl;
        return false;
    }
